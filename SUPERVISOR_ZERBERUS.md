...
*Letzte Aktualisierung: P-debt-12 (2026-05-10) — Leitregel-Anker in `docs/DESIGN.md` wiederhergestellt (Schulden #11, ein-Zeilen-Wartungs-Patch). Bei P-UI-meta (2026-05-07) wurde DESIGN.md komplett neu gefasst, dabei gingen die test-zementierten Anker `Leitregel`, `projektübergreifend` und `shared-design.css` verloren — `test_design_system::test_design_md_enthaelt_regel` blieb seit drei Wochen rot. Fix: kompakter `## Leitregel`-Block direkt nach der Einleitung wieder eingefuegt, der die ursprungliche Spec aus Patch 151 (L-001) ehrlich re-interpretiert und alle drei Anker organisch im Inhalt traegt. **Schulden #9 (Hel-Splitscreen) am 2026-05-10 von Chris als visuell GELOEST bestaetigt** — der zuvor zur Haelfte schwarze Splitscreen-Bereich ist behoben (`UI_BUG_HEL_SPLITSCREEN.md` auf GELOEST gesetzt). 12/12 `test_design_system` gruen, 426 Tests in der UI-/Hel-/Phase-5c-/huginn-doc-Suite gruen, 10 pre-existing Failures (7× `test_patch185_runtime_info` config.yaml-Worktree-Drift Schulden #6, 3× `test_p210_huginn_rag_sync::TestDocSourceAudit` neue Schulden #12 wegen Stand-Anker-Format `P-UI-Hel-Split` matched die `P\d{3,4}`-Regex nicht). Phase 5a + Phase 5c bleiben VOLLSTAENDIG ABGESCHLOSSEN. Letzter UI-Bugfix-Patch P-UI-Hel-Split. Vorletzter Maintenance-Patch P-debt-11. Letzter Backend-Bugfix-Patch P217. Letzter Code-Patch mit UI-Mechanik P-UI-11.*

---

## Letzter Maintenance-Patch (Doku, Anker-Reparatur)
...
**Status-Updates parallel.** **Schulden #9 (Hel-Splitscreen) auf GELOEST gesetzt** — Chris hat am 2026-05-10 visuell verifiziert: "Split Screen habe ich getestet. Funktioniert. […] der Split-Screen war zur Haelfte schwarz und das ist jetzt nicht mehr." `UI_BUG_HEL_SPLITSCREEN.md` aktualisiert. **Neue Schulden #12 dokumentiert** — `test_p210_huginn_rag_sync::TestDocSourceAudit::{test_main_doc_has_stand_anker, test_main_doc_patch_is_recent, test_mirror_doc_has_stand_anker}` blockieren auf der Regex `\*\*Letzter Patch:\*\*\s+(P\d{3,4})\b` in [`tools/sync_huginn_rag.py:138`](tools/sync_huginn_rag.py:138). Patch-Namen mit Buchstaben-Suffix wie `P-UI-Hel-Split`, `P-UI-11`, `P-debt-11`, `P-debt-12` matchen nicht. Niedriger Aufwand: Regex auf `r"\*\*Letzter Patch:\*\*\s+(P[\w-]+)\b"` erweitern oder den Stand-Anker auf ein P###-faehiges Format umstellen.
