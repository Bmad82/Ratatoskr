...
<!-- STAND-ANKER:START -->
**Stand:** FR 2026-05-30 Master-Roadmap Block A+B+C (CodeCat-401-Fix + Dictate-Sentiment-Skip + Marathon-Auffuell-Regel 400k + Master-Queue) - Phase 5b - 2026-05-30 - ~4820 Tests
...
**Externes 4-Modell-Code-Review (Code-Dossier, 2026-06-13)** — KEIN Code-Patch an `zerberus/**`, sondern eine unabhängige Begutachtung der gesamten Codebasis durch vier fremde LLMs über OpenRouter. Werkzeug: neue Factory `Zerberus_Codedossier.py` (abgeleitet aus `code_dossier-1.py`, umgebaut auf Single-Package `pkg_full` + `CONTEXT.md` + SKP-Fragen; `__init__.py`-mit-Logik wird eingeschlossen, `.bak`/Backups/Archive geskippt). Das Paket (167 Dateien, ~64k Zeilen, ~658k Tokens) ging in je einem frischen, blinden Call an `gemini-3-flash` (reasoning high), `grok-4.3`, `deepseek-v4-pro`, `gpt-5.4-pro` (temperature 0.2). **Ergebnis:** DeepSeek + Gemini lieferten code-fundierte Befunde mit Datei:Zeile (hohes Signal), Grok nur Zeilenzahlen (niedrig), **gpt-5.4-pro lieferte 0 Zeichen** — verbrauchte alle 9000 `max_tokens` intern als Reasoning (`finish_reason=length`) und erreichte die Finalantwort nie ($42,21 ohne Output). **Verifikation am echten Code** widerlegte die bug-förmigen Befunde: SQL-Injection in `database.py` = False-Positive (alles `text()`/ORM); `/archive/`-Auth-frei = bewusste E-02/Tailscale-Entscheidung; RAG `_reset_index_inplace` = dokumentierter P217/P218-Crash-Guard (B-023), kein Datenverlust-Bug; `orchestrator.py` = KEIN toter Code (deckt sich mit CodeCat-Inventur Punkt 4), sondern aktiver Provider via deferred Import; Telegram-Backoff IST gedeckelt (Grok lag falsch). **Cross-validierte echte Schulden** (Refactor, nicht Review-Seiteneffekt): chat_pipeline.py God-Object (4/4 Modelle, **B-101**), orchestrator-Circular-Import (**B-102**), Inline-Frontend in Hel. **Keine Reparatur an Produktivcode** — kein bestätigter, sicher behebbarer Defekt; nur das Review-Artefakt `CONTEXT.md` korrigiert (orchestrator-Prämisse war faktisch falsch) + Factory-Template synchron. Kosten gesamt **$44,58** (deepseek $0,30 | gemini $0,40 | grok $1,67 | gpt $42,21). Voller Bericht: [`code_dossier/REVIEW_SUMMARY_2026-06-13.md`](code_dossier/REVIEW_SUMMARY_2026-06-13.md). **Netto:** Das Review fand keinen neuen Defekt — starkes Reife-Signal (≈220 Patches, ~2900 Tests). **KEIN Server-Restart** (kein `zerberus/**`-Touch). **Lehre für Re-Runs:** Reasoning-Heavy-Modelle brauchen `max_tokens` ≥25k oder `reasoning.effort` niedrig, sonst Geld ohne Output.

## Vorheriger Patch

**FR Bildarchiv + Bild-Upload-Bugfix (P-FR-Bildarchiv, 2026-06-07)** — Top-Prio-FR vor allen Phase-5b-Items (Vorbereitung Side-Urlaub 2026). Vier Schritte in EINER Coding-Session laut OVERRIDE (ein Commit pro Schritt, ein Sammel-HANDOVER). **Schritt 0 ([IMGFIX]):** Nala-Bild-Upload schickte das Bild bisher sofort, ohne Begleittext. Jetzt: Bild wird als Thumbnail-Preview (max 80px, 44px-Touch-Targets, mobile-first) angeheftet, Text bleibt eingebbar, Senden schickt Bild+Text zusammen, X-Button entfernt das angeheftete Bild, Senden-ohne-Text schickt nur das Bild. Files: `nala.py` (Preview-Bar im `.input-area`), `nala.css` (`.image-preview-*`), `nala-main.js` (`_pendingImage`/`renderImagePreview`/`clearPendingImage`/`sendImageMessage`). **Schritt 1a ([IMGARCHIVE]):** Projekt-Bild-Upload (MIME jpeg/png/webp/gif, Endung als Fallback) erzeugt automatisch eine deutsche Vision-Beschreibung (OpenRouter via vorhandenem `analyze_image`, fester Prompt ohne Namen), legt sie als `<sha>_beschreibung.txt` neben den SHA-Bytes ab (inhalts-adressiert, dedup-geteilt), persistiert sie in der NEUEN Spalte `project_files.vision_description` (graceful `ALTER TABLE` in `init_db`, PRAGMA-Guard) und indexiert sie als EINEN RAG-Chunk (`source="image:<relative_path>"`, `chunk_type="vision_description"`, idempotent per `file_id`). **Best-Effort:** Vision-Ausfall (down/Timeout/kein Key) bricht den Upload NICHT ab — Beschreibung bleibt NULL und ist via Hel-Button nachholbar. Neue Datei `zerberus/core/projects_vision.py`; `database.py` (Spalte+Migration), `projects_repo.py` (`set_vision_description`), `projects_rag.py` (`index_image_description` + `format_rag_block`-Label). **Schritt 1b:** Hel-Dateiliste mit Thumbnails (`GET /admin/projects/{pid}/files/{fid}/raw`, volle Bytes, CSS begrenzt auf max 120px), Beschreibung unter dem Dateinamen (expandierbar), "Beschreibung neu generieren"-Button (`POST .../describe`, `force=True`) und "Nur Bilder"-Filter — Rendering ausgelagert nach `_renderProjectFilesList()` in `hel-main.js`. **Schritt 1c:** RAG-Block markiert Bild-Treffer mit `📷 ... (Bild-Beschreibung)` damit das LLM weiß, dass ein ladbares Bild dahinter liegt; explizites "zeig mir/schau dir an" → Bild-Reload an Vision ist dokumentierter Bonus für v2 (Beschreibung reicht für v1, kein Regress-Risiko auf dem Daily-Driver-Chat-Hotpath). **Tests:** `test_imgfix_attach_then_send.py` (14) + `test_imgarchive_vision_bridge.py` (10, Vision+Embedder gemockt) grün; Regression-Sweep `nala/project/rag/phase5b/vision` grün. Migration empirisch gegen Kopie der Live-`bunker_memory.db` + synthetisches Alt-Schema getraced (ALTER erhält Bestandsdaten, NULL für Altzeilen). 3 pre-existing Doc-Audit-Failures (`test_p210`) als Nebeneffekt mitgefixt (Stand-Anker hatte keine P-Form). **Live-Deploy triggert Auto-Restart** (Code-Änderungen unter `zerberus/**`). **Heilige Regel:** Vision ist Best-Effort — niemals den Upload-Pfad blockieren.

## Vorvorheriger Patch

**FR 2026-05-30 Master-Roadmap — Block A+B+C (Marathon-Start, 2026-05-30)** — Erste Session des Multi-Session-FR. Atomaritaets-Override (Block A+B+C in EINER Session). **Block A (CodeCat-401-Hotfix):** `/archive/` zur `_JWT_EXCLUDED_PREFIXES`-Liste in `zerberus/core/middleware.py:95-110` hinzugefuegt — analog zu `/code-cat` (PATH-03). CodeCat-Frontend ruft `/archive/sessions?limit=30` mit `credentials:'same-origin'` ohne JWT → vorher 401, jetzt OK. Tailscale-Trust deckt /archive/* ab (internes Desktop-Tool). Regression-Test in `test_fr_2026_05_30_block_a_archive_auth_bypass.py` (6 Tests) zementiert: /archive/ steht im Block, Exclusion-Schleife laeuft VOR `verify_token`, /v1/ und /code-cat bleiben unveraendert (E-08-Invariante). **Block B (Dictate-Sentiment-Skip Regression):** Live-Log 2026-05-30 zeigte BERT-Lazy-Load (`Lade german-sentiment-bert (device=cuda)`) bei reinem Diktieren. Root-Cause: `store_interaction("whisper_input", raw_transcript, …)` (legacy.py:306) ruft `_compute_sentiment(content)` in `database.py:564` → triggert analyze_sentiment → BERT-Load. B-075 hatte den OBERFLAECHEN-Call entfernt, den indirekten via store_interaction uebersehen. Fix: in `store_interaction` wird fuer `role == "whisper_input"` der Sentiment-Branch uebersprungen (sentiment=None). Whisper_input ist rohes Diagnose-Logging (Pausen, "aeh", verschluckte Worte) — Sentiment macht semantisch keinen Sinn. Regression-Test `test_fr_2026_05_30_block_b_whisper_input_sentiment_skip.py` (6 Tests): source-audit + Verhaltens-Test mit Sentinel-Exception, Anti-Drift-Test fuer role="user". **Block C (MARATHON-Workflow):** Auffuell-Regel auf neue Schwellen kalibriert — `< 400k` → naechstes Master-Queue-Item, `~400k` → STOP fuer Neues + Doku/HANDOVER/Push, `~450k` harte Reserve. Alte ~350k-Stopp-Schwelle (50k Reserve) hat den Marathon zu frueh gebremst. Master-Queue als persistente, priorisierte 5-Tier-Liste verankert (Q-01 Root-Cleanup, Q-02 Backlog-Inventur, Q-10..Q-13 CodeCat-Folge-FRs, Q-20..Q-22 Rosa-Readiness-Reste, Q-30..Q-39 Backlog-Restbestand, Q-50..Q-56 Phase-5b Power-Features). Eskalations-Regel: Coda fragt Chris NUR bei echten Architektur-Risiken, physisch-unmoeglichen Tests, `--execute`-Pflicht (FAISS B-010). 9-Schritt-Session-Zyklus unangetastet. **Multi-Session-STATUS:** FR ist absichtlich mehrsitzungsfaehig — solange Master-Queue nicht leer ist, traegt mjolnir.md `STATUS: IN_ARBEIT`, FR-Datei NICHT umbenennen. **Tests:** 12 neue Block-A/B-Tests gruen, 27 verwandte B-075/PATH-03-Tests gruen, 20 db_helpers-Tests unveraendert. **Auto-Restart-Hook (deploy_to_live.ps1) wird beim Sync triggern** — Code-Aenderungen unter `zerberus/**` (middleware.py + database.py).
