...
*Letzte Aktualisierung: P-UI-Hel-Split (2026-05-09) — Defensive CSS-Hardening fuer Hel-UI im Splitscreen-Modus (Schulden #9). Vier Klassen Edits in `zerberus/app/routers/hel.py` Style-Block: (a) html-Layer mit `box-sizing: border-box; min-height: 100%; overflow-x: hidden;` als Boden-Sperre fuer horizontalen Overflow, (b) body mit `min-height: 100vh; overflow-x: hidden;` als zweite Schicht, (c) `.container` mit explizitem `width: 100%` (Edge Snap-Layout-Quirk-Defense), (d) neuer `@media (max-width: 900px)`-Breakpoint der Body/Section/Card-Padding reduziert, Tab-Nav-Margin/-Padding auf -12px korrigiert, `.container > * { min-width: 0 }` setzt (Flex-/Grid-Items-Schrumpf-Befreiung) und Inline-Style-Grids `1fr 1fr 1fr` + `1fr 1fr` auf 1 Spalte stackt. Plus `#messagesTable td { max-width: clamp(120px, 32vw, 200px) }` damit das 200-px-Cell-Limit keine kuenstliche min-content im Splitscreen erzwingt. Neuer Test-File `test_p_ui_hel_split.py` mit 36 Source-Audit-Tests in 12 Klassen — 36/36 lokal gruen. UI-relevante Test-Suite (10 Hel-bezogene Files + 7 P-UI-Files): 286/286 gruen, 1 pre-existing Failure in `test_design_system::test_design_md_enthaelt_regel` (sucht `Leitregel`/`projektuebergreifend` in DESIGN.md, **nicht durch P-UI-Hel-Split verursacht** — via git-stash-Verifikation bestaetigt). Phase 5a + Phase 5c bleiben VOLLSTAENDIG ABGESCHLOSSEN. Vorletzter Maintenance-Patch P-debt-11. Letzter Backend-Bugfix-Patch P217. Letzter Code-Patch mit UI-Mechanik P-UI-11. **Visueller Verifikations-Schritt fuer Chris:** Hel im Win+Pfeil-Splitscreen-Modus oeffnen — pruefen, ob die Mitte noch abgeschnitten erscheint. Falls ja: eigener Folge-Patch P-UI-Hel-Split-2 mit gezielterer Diagnose (live-screenshot via Chris, da Coda-Worktree keine Chrome-Extension verbunden hat).*
