...
*Letzte Aktualisierung: P217 (2026-05-09) — Globaler RAG: Sprach-Routing (DE/EN-Slot-Trennung) + Chunk-Orphan-Bereinigung beim DELETE. Backend-Bugfix-Patch ausserhalb der Phase-Ziele, behebt zwei zusammenhaengende Bugs im globalen RAG-Pfad. Bug 1: vor P217 routete `_add_to_index` jeden Vektor in den globalen DE-Slot — egal welche Sprache. Multi-Chunk-Upload mit DualEmbedder (DE=768/EN=1024) zerstoerte den Index per `_reset_index_inplace` bei jedem Sprachwechsel (P218-pre-Recovery). Bug 2: `rag_document_delete` setzte nur DE-Soft-Delete + ohne physischen Reindex — EN-Chunks blieben Orphans, Hel-Status zeigte verwaiste Chunks. P217 macht `_add_to_index`/`_reset_index_inplace`/`_rebuild_index_atomic` sprach-aware (`language`-Param), `_resolve_paths` bekommt `for_write=True`-Param fuer strikte Sprach-Anwendung beim Schreiben, DELETE iteriert DE+EN und triggert physischen Reindex pro Slot, `_reset_sync` leert auch en.index auf Disk wenn die Datei existiert, `rag_status`/`rag_documents` aggregieren DE+EN. Edits in [`zerberus/modules/rag/router.py`](zerberus/modules/rag/router.py) (5 Helper sprach-aware) + [`zerberus/app/routers/hel.py`](zerberus/app/routers/hel.py) (UPLOAD/DELETE/STATUS/DOCUMENTS). Neuer Test-File [`test_p217_index_persistence.py`](zerberus/tests/test_p217_index_persistence.py) mit **28 Tests** in 8 Klassen — Sprach-Routing, kein Cross-Language-Reset, Slot-Init, for_write-Param, Reindex-Atomic-Language, Reset-Sync-EN-Disk, Source-Audit, Status-Aggregation. Bestehende Tests aktualisiert: zwei Source-Audit-Substring-Asserts in `test_p213_pre_2_dual_storage.py` flexibler, `test_empty_chunks_is_noop` umbenannt zu `test_empty_chunks_purges_slot` mit gespiegelten Asserts (Spec-Verschiebung: leere Chunks purgen den Slot fuer DELETE-Pfad). Volle Unit-Suite-Erwartung **3398 passed** (P-UI-Polish-2-Baseline 3381 + 28 P217-Tests minus alte Erwartungen die jetzt anderes Verhalten haben). Phase 5a + Phase 5c bleiben VOLLSTÄNDIG ABGESCHLOSSEN — P217 ist Backend-Bugfix-Patch ausserhalb der Phase-Ziele. Vorletzter Patch (Cosmetic-Polish) unveraendert P-UI-Polish-2. Letzter Code-Patch mit UI-Mechanik unveraendert P-UI-11.*
