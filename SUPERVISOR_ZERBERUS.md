...
*Letzte Aktualisierung: P-UI-Polish (2026-05-09) — Phase-5c-Polish: visuelles Fine-Tuning nach Mockup-Vorbild [`docs/NalaMockup.jsx`](docs/NalaMockup.jsx), KEINE strukturelle Aenderung. Reine CSS-Wert-Aenderungen in [`zerberus/app/routers/nala.py`](zerberus/app/routers/nala.py): User-Bubble dezenter (rgba 0.88 → 0.12 + Pink-border-left + 10px-Radius), Bot-Bubble komplett entfernt (transparent + radius 0 + shadow none — freistehender Text wie ChatGPT/Claude/Mistral), Sidebar-Karten schlanker (Mistral-Pattern), Eingabefeld dezenter, Chat atmet (gap 12 → 18). DESIGN.md Sektion 1.6 + neue Anti-Invariante 2 ("Bot ohne Bubble-Hintergrund"). UI-relevante Test-Suite **723/723 gruen**. Volle Unit-Suite-Erwartung **3315 passed** (P-UI-11-Baseline 3270 + 40 P-UI-Polish-Tests + 5 Netto). Phase 5a + Phase 5c bleiben VOLLSTÄNDIG ABGESCHLOSSEN — P-UI-Polish ist Cosmetic-Folge-Patch ausserhalb der Phase-Ziele. Letzter Maintenance-Patch unveraendert P-debt-10. Letzter Code-Patch (mit Mechanik) unveraendert P-UI-11.*
