...
*Letzte Aktualisierung: FEATURE_REQUEST_ZERBERUS Komponenten-Migration Sammel-Session N+5..N+10 — Kintsugi-Typografie auf Reasoning + Input + Scroll-Nav + Project-View + Settings + Final Cleanup (2026-05-21). **STATUS: FERTIG** (alle 10 Sub-Sessions abgeschlossen; FEATURE_REQUEST_ZERBERUS.md umbenannt zu FEATURE_REQUEST_ZERBERUS_ERLEDIGT.md). Geliefert in einer Coding-Session laut OVERRIDE-Direktive: Sub-Sessions sind logische Arbeitspakete, EIN Commit pro Sub-Session, EIN Sammel-HANDOVER. Pattern in jeder Sub-Session identisch zu N+1..N+4: Class-Annex statt Rename — additive `var(--zb-font-*, inherit)`-Properties, keine HTML/JS/ID-Touches, kein Anfassen Heiliger Anker. **N+5 (Commit `e3cb639`):** `.pui6-reasoning-toggle` + `.pui6-reasoning-label` + `.reasoning-toggle` (Mono-Caps 0.18em). **N+6 (`b75739f`):** `#text-input` Body-Font + Placeholder Italic. **N+7 (`40b44a1`):** Scroll-Nav-Active-Oval Token-Nesting `var(--zb-gold, var(--color-gold, #f0b429))`. **N+8 (`1875350`):** `.projects-header-title` Display-Font + `.projects-pinned-heading` + `.projects-regular-heading` Mono-Caps + `.project-card-name` Body-Font. **N+9 (`0bc73b4`):** `.settings-tab-btn` + `.settings-section h5` Mono-Caps + `.theme-modal-inner h4` + `.export-modal-inner h4` Display-Font. **N+10 (`c241099`):** Mobile-Check (66+13 Tests PASSED ueber alle vorherigen Sub-Sessions) + Final Cleanup — `zerberus/static/css/nala-kintsugi.css` (1145 Z.) + `zerberus/static/js/nala-kintsugi-prototype.js` (394 Z.) per `git rm` geloescht (nie via `<link>`/`<script>` geladen, verwaiste Prototyp-Dateien aus Claude-Design-Import). **Test-Ergebnis Mega-Regression: 759/759 PASSED** in 12+ Suites (`test_p_ui_*`, `test_nala_*`, `test_frontend_separation`, `test_settings_umbau`, `test_design_system`, `test_p203d3_nala_code_render`, `test_nala_adapter`). Pre-existing 4 Failures in `test_nala_projects_tab` (PROJECTS-env-Pydantic-Baseline) unveraendert. nala.css 2640 → 2661 Z. (+21 Z. additiv, unter 3000-Limit). Migrationsbasis Kintsugi-Foundation jetzt komplett auf Reasoning/Input/Scroll-Nav/Project-View/Settings — Folge-Sessions koennen die `nala-design.css`-Token-Definitionen ausweiten. Vorheriger Stand: FEATURE_REQUEST_ZERBERUS Komponenten-Migration Session N+4 — Chat-List + Bubbles-Kintsugi-Typografie (2026-05-21). **STATUS: IN_ARBEIT** (FEATURE_REQUEST bleibt liegen — 6 Sub-Sessions N+5 bis N+10 stehen aus). Geliefert: `zerberus/static/css/nala.css` 3 Selektoren additiv ergaenzt — `.message` bekommt `font-family: var(--zb-font-body, inherit);` (Crimson-Body fuer alle Bubbles via Vererbung), `.expand-toggle` ("Mehr"-Button P124) bekommt vollen Mono-Caps-Treatment (`font-family: var(--zb-font-mono, inherit); text-transform: uppercase; letter-spacing: 0.22em;`), `.message .bubble-preview` (P-UI-2 Collapsed-Preview) bekommt nur `font-family: var(--zb-font-body, inherit);` (Italic + Ellipsis bleiben). Pattern identisch zu N+1 (Login-Polish), N+2 (Header), N+3 (Sidebar): CSS-only, additive Properties, keine HTML/JS/ID-Touches, Fallback `inherit` ohne Foundation. **Audit ergab:** Plan-Anker fuer N+4 (`<ol class="chat-list">`-Wrapper, Dual-Class `.msg-bot.bot-message`, `msg-avatar`/`msg-content`/`msg-body`-Sub-Struktur, Risiko SEHR HOCH durch zentrale `addMessage()`-Append-Funktion + ~30 JS-Hooks) sind HTML-Strukturaenderung — bewusst nicht angefasst (Class-Annex-Pattern umgeht das Risiko vollstaendig). Reale Bubble-DOM hat `#chatMessages`, `.chat-messages`, `.msg-wrapper`, `.message`, `.bot-message`, `.user-message`, `.bubble-preview`, `.bubble-full`, `.bubble-collapse-toggle`, `.expand-toggle`, `.msg-toolbar`, `.bubble-action-btn`, `.copy-btn`. Lesson aus N+2/N+3 bestaetigt: immer zuerst Real-Code-Audit, dann erst der Plan. **Test-Ergebnis: 0 neue Regressions** — `test_frontend_separation` 8/8 + `test_nala_bubble_layout` 13/13 + `test_p_ui_1_layout` 24/24 + `test_p_ui_2_collapse` 24/24 + `test_p_ui_polish_visual_weight` 40/40 = 101/101 Core-PASSED. Erweiterter Lauf (547/547 in 12 Adjazent-Suites: `test_p_ui_polish_2_layout`, `test_p_ui_4_sidebar_content_shift`, `test_design_system`, `test_p_ui_10_sentiment_ambient`, `test_p_ui_11_reasoning_mapping`, `test_p_ui_6_reasoning_block`, `test_p_ui_3_textarea_collapse`, `test_p_ui_8_scroll_nav`, `test_p_ui_5_project_view`, `test_settings_umbau`, `test_p203d3_nala_code_render`, `test_nala_adapter`) bestaetigt 0 neue Regressions. nala.css 2640 Z. (unter 3000-Limit, +5 Z. gegenueber N+3). Vorheriger Stand: FEATURE_REQUEST_ZERBERUS Komponenten-Migration Session N+3 — Sidebar-Kintsugi-Typografie (2026-05-21). **STATUS: IN_ARBEIT** (FEATURE_REQUEST bleibt liegen — 7 Sub-Sessions N+4 bis N+10 stehen aus). Geliefert: `zerberus/static/css/nala.css` 2 Selektoren additiv ergaenzt — `.sidebar h3` (Pinned-Heading „📌 Angepinnt") bekommt vollen Mono-Caps-Treatment (`font-family: var(--zb-font-mono, inherit); text-transform: uppercase; letter-spacing: 0.18em; font-size: 11px; font-weight: 600;`), `.sidebar-chats-heading` (CHATS-Caption) bekommt nur `font-family: var(--zb-font-mono, inherit);` (vorhandene Caps-Properties bleiben). Pattern identisch zu N+1 (Login-Polish) und N+2 (Header-Migration): CSS-only, additive Properties, keine HTML/JS/ID-Touches, Fallback `inherit` ohne Foundation. **Audit ergab:** Plan-Anker „Sidebar-Header mit Heart-Asset" sind HTML-Strukturaenderung — bewusst nicht angefasst (passt nicht ins Class-Annex-Pattern). Reale Sidebar-DOM hat `#sidebar`, `.sidebar-header`, `.sidebar-actions`, `.sidebar-action-btn`, `.sidebar-save-btn`, `.sidebar-projekte-row`, `.sidebar-separator`, `.sidebar-projects-list`, `.sidebar-project-item`, `.sidebar-chats-heading`, `#pinned-heading`/`.sidebar h3`, `.session-list`, `.session-item`, `.sidebar-footer*`. Lesson aus N+2 bestaetigt: immer zuerst Real-Code-Audit, dann erst der Plan. **Test-Ergebnis: 0 neue Regressions** — `test_frontend_separation` 8/8 + `test_p_ui_polish_2_layout` 66/66 + `test_p_ui_4_sidebar_content_shift` 29/29 + `test_p_ui_polish_visual_weight` 40/40 + `test_design_system` 12/12 = 155/155 PASSED. Erweiterter Lauf (298/298 in 7 Adjazent-Suites: `test_nala_bubble_layout`, `test_p_ui_5_project_view`, `test_nala_adapter`, `test_settings_umbau`, `test_p_ui_10_sentiment_ambient`, `test_p_ui_11_reasoning_mapping`, `test_p203d3_nala_code_render`) bestaetigt 0 neue Regressions. nala.css 2635 Z. (unter 3000-Limit, +14 Z. gegenueber N+2). Vorheriger Stand: FEATURE_REQUEST_ZERBERUS Komponenten-Migration Session N+2 — Header-Kintsugi-Typografie (2026-05-21, Session 3/3). **STATUS: IN_ARBEIT** (FEATURE_REQUEST bleibt liegen — 8 Sub-Sessions N+3 bis N+10 stehen aus). Geliefert: `zerberus/static/css/nala.css` 3 Selektoren additiv ergaenzt — `.title { font-family: var(--zb-font-display, inherit); letter-spacing: -0.01em; }` (Playfair Display fuer Brand-Headline `👑 Nala`), `.profile-badge { font-family: var(--zb-font-mono, inherit); font-style: normal; letter-spacing: 0.18em; text-transform: uppercase; }` (JetBrains-Mono-Caps fuer Username-Subtitle), `.active-project-chip` identische Mono-Caps-Treatment (Patch-201-Chip). Pattern identisch zu N+1 (Login-Polish): CSS-only, additive Properties, keine HTML/JS/ID-Touches, Fallback `inherit` ohne Foundation. **Audit ergab:** Plan-Anker `#topbar/.session-title/.session-subtitle` in `docs/NALA_KINTSUGI_MIGRATION_PLAN.md` **existieren nicht in der Produktion** — reale Header-DOM nutzt `.header/#main-header/.title/.profile-badge/#active-project-chip`. Lesson fuer Folge-Sessions: immer zuerst Real-Code-Audit, dann erst der Plan. **Test-Ergebnis: 0 neue Regressions** — `test_frontend_separation` 8/8 + `test_p_ui_polish_2_layout` 41/41 + `test_nala_bubble_layout` 21/21 + `test_design_system` 17/17 + `test_p_ui_5_project_view` 67/67 PASSED. Erweiterter Lauf (290/294) bestaetigt nur 4 pre-existing PROJECTS-env-Pydantic-Failures, Baseline-Diff via `git stash` validiert. nala.css 2621 Z. (unter 3000-Limit). Vorheriger Stand: FEATURE_REQUEST_ZERBERUS Nachtrag — Gist-Sync-Aufholarbeit + Migration-Plan + Login-Polish (2026-05-21, Session 2/2). **STATUS: IN_ARBEIT** (FEATURE_REQUEST bleibt liegen — Komponenten-Migration laeuft jetzt nach `docs/NALA_KINTSUGI_MIGRATION_PLAN.md` in 10 Sub-Sessions, Login (N+1) erledigt). Geliefert: (1) **Gist-Sync-Aufholarbeit** — Audit `git log` der letzten 4 Sessions zeigt: B-074 (e080f90), Nala-Frontend-Separation (f4f689e), Hel-Frontend-Separation (cec3a76), FEATURE_REQUEST Teil-Lieferung (b7dcd08) haben alle Schritt 12 (Gist-Sync) uebersprungen — Faulheits-Catch #2 Derivat. **Lesson** am Top von `lessons_ZERBERUS.md` + `C:\Users\chris\Python\Claude\GLOBAL_LESSONS.md` zementiert (Pipe-Format-Header + Anlass + Loesung + generalisierbar). Claude-Repo Commit `9da822a` + push erfolgt. **Zerberus-Gist** `7f5af9dd878a6a9d664f062976b27ae8` per GitHub-API-PATCH aktualisiert (HTTP 200): `STATUS.md` 874 B aus mjolnir-Header generiert, `HANDOVER.md` 16030 B, `MJOLNIR.md` 12012 B, `LESSONS.md` 316303 B, `REPO_INDEX.md` 18737 B. **Claude-KB-Gist** `48b997e53ff331eeefef53c810ee7331` per PATCH (HTTP 200): `GLOBAL_LESSONS.md` 16920 B. Token via `git credential fill`, PATCH-Skripte inline (`/tmp/gist_patch.py` + `/tmp/gist_kb_patch.py`) — gist_publisher.py kann nur `create`. (2) **Multi-Session-Migration-Plan** `docs/NALA_KINTSUGI_MIGRATION_PLAN.md` geschrieben — zerlegt verbleibende Komponenten-Migration in 10 fokussierte Sub-Sessions (N+1 Login, N+2 Header, N+3 Sidebar, N+4 Chat-List+Bubbles, N+5 Reasoning/Code/Output/HitL, N+6 Input-Bar, N+7 Scroll-Nav, N+8 Project-View, N+9 Settings-Modal, N+10 Final). Strategie: Class-Annex statt Class-Rename (additive Selektor-Properties), Heilige Anker enumeriert (`#chatMessages`, `#text-input`, `#sendBtn`, `#micBtn`, `#imageBtn`, `#login-username`, Patch-119–147-Anker, etc.), pro Session Baseline-Test-Diff vor + nach mit 0-Regression-Pflicht. (3) **Cleanup** — `docs/Zerberus.zip` (tracked, 176 KB) per `git rm` entfernt (Inhalt liegt entpackt im Repo); `.zerberus_zip_tmp/` (gitignored, 727 KB Arbeitskopie) per `rm -rf` entfernt. (4) **Login-Polish als erster Komponenten-Schritt (N+1)** — `zerberus/static/css/nala.css` 2 Selektoren additiv ergaenzt: `#login-screen h2` bekommt `font-family: var(--zb-font-display, inherit); font-weight: 700; letter-spacing: -0.01em;` (Playfair Display wenn Foundation-Layer geladen, `inherit` als Fallback), `.login-hint` bekommt `font-family: var(--zb-font-mono, inherit); letter-spacing: 0.22em; text-transform: uppercase;` (JetBrains-Mono-Caps-Stil). Test-Anker-Check vorher (`grep -rln "login-screen h2|login-hint"`) ergab keine Test-Anker — sichere Aenderung. nala.css 2608 Z. (unter 3000-Limit). **Test-Ergebnis: 0 neue Regressions** — `test_frontend_separation` 8/8 PASSED, `test_login` + `test_design` + `test_kintsugi` + `test_nala_source` 108/108 PASSED. Voll-Suite nicht erneut gefahren (Baseline 45/3401/108 ungefaehrdet durch 2-Selektor-additive-CSS-Aenderung ohne Backend-Beruehrung). FEATURE_REQUEST_ZERBERUS.md **bleibt liegen** (9 Sub-Sessions stehen noch aus, N+2 bis N+10). Schritt 12 (Gist-Sync) ist ab jetzt Pflicht — auch wenn Tokens knapp werden. Vorheriger Stand: FEATURE_REQUEST_ZERBERUS Teil-Lieferung (2026-05-20 → 2026-05-21) — Multi-Session-Auftrag. **STATUS: IN_ARBEIT** (nur Kintsugi-Komponent-Migration vertagt — alle Bugs + Feature komplett). Geliefert: (1) **F-01 Nala Bild-Upload-Button** — `<button class="image-btn" id="imageBtn">📷</button>` in `nala.py`-NALA_HTML, neuer Endpoint `POST /nala/image` (UploadFile + Form-Prompt, 10 MB Limit, ruft `analyze_image()` aus `zerberus/utils/vision.py` mit `qwen/qwen2.5-vl-7b-instruct`), JS-Handler in `nala-main.js` (FormData-Submit, 401-Logout, 413-Toast, processing-State), CSS `.image-btn` in `nala.css` (Touch-Target ≥44px via 50×50 px, Gold-Accent-Hover, `pulseGold`-Animation waehrend Processing). (2) **B-01 Whisper-Cleaner ARD-Funk-Hallucination** — `whisper_cleaner.json` +2 Regex (Komplettzeile `^\s*ard\s+text\s+im\s+auftrag\s+von\s+funk\.?\s*$` und inline-Variante `\bard\s+text\s+im\s+auftrag\s+von\s+funk\.?`), beide case-insensitive, direkt nach ZDF-Pattern-Block eingefuegt — Reihenfolge ist signifikant. (3) **B-02 Hel LLM-Tab Kontostandanzeige** — Backend (`hel.py::get_balance`) faellt bei `limit_usd is None` (Pre-paid-OpenRouter-Accounts) auf `/credits`-Endpoint (`total_credits`/`total_usage`); Frontend (`hel-main.js`) schuetzt USD-Suffix vor `null.toFixed`-TypeError per `Number.isNaN`-Guard. (4) **B-03 Hel LLM-Tab Sortierung nach Name** — in `hel-main.js::renderModelSelect` Locale-aware `localeCompare('de', { sensitivity: 'base' })` aktiviert wenn `_currentSort === 'name'` (vorher identischer Code wie Preis-Pfad, faktisch tot). „Patch 147"-Comment-Anker wiederhergestellt fuer Test-Stabilitaet. (5) **B-04 Huginn 409 Conflict / Zombie-Prozesse Dauerfix** — PID-Lock-File `data/huginn_polling.pid` + `acquire_polling_lock()`/`release_polling_lock()` in `zerberus/modules/telegram/bot.py` mit cross-platform `_pid_is_alive()` (Windows: `OpenProcess`+`GetExitCodeProcess`+`STILL_ACTIVE=259`; Unix: `os.kill(pid, 0)`); Lock-Acquire in `startup_huginn()` vor `asyncio.create_task(long_polling_loop(...))`; `try/finally`-Release im Polling-Loop; 409-spezifischer 15s-Backoff mit Single-WARN-Suppression; `.gitignore`-Eintrag `data/huginn_polling.pid`. **Nur Design-Komponent-Migration vertagt** (Foundation-Layer geliefert): Claude-Design-Export aus `docs/Zerberus.zip` (Kintsugi Void/Gold/Ember-Palette) ist UI-only-Prototyp ohne Backend-Wiring (kein SSE, kein Voice, kein jsPDF, kein Pacemaker). Erkenntnis: blindes Mergen wuerde alle Nala-Backend-Pfade brechen. Loesung: Kintsugi-Tokens isoliert in neuer `nala-design.css` (NICHT in `shared-design.css` — Hel nutzt die alten `--zb-primary`-Tokens!), `nala.py` laed sie NACH `nala.css` (Cascade-Override nur auf Nala). Prototyp-Files als Read-Only-Referenz behalten (`nala-kintsugi.css`, `nala-kintsugi-prototype.js`, plus Assets `kintsugi-heart.svg`/`noise-filter.svg`/`seam-divider.svg` — NICHT geladen). Komponent-Migration (Sidebar/Project-View/Settings-Panel-Refactor mit neuen Class-Names + IDs + Backend-Re-Wiring) folgt in Anschluss-Session. **Test-Ergebnis: 0 neue Regressions** (Baseline 45/3401 → 45/3401, identische Failure-Liste per Set-Diff verifiziert). Kein FEATURE_REQUEST-Rename in dieser Session (Multi-Session-Regel, Folge-Session braucht den Auftrag). Vorheriger Stand: Hel-Frontend-Separation (2026-05-20) — `hel.py` 6.576 → 3.604 Zeilen. Inline-CSS (~420) + 4 JS-Bloecke (~2.558) extrahiert nach `zerberus/static/css/hel.css` und `zerberus/static/js/hel-{fouc,main,gpu-toast,pwa-sw}.js`. ADMIN_HTML referenziert die Statics via `<link>` / `<script src>`. 16 Source-Audit-Tests via neuem Shared-Helper `zerberus/tests/_hel_source.py` migriert; 2 `node --check`-Tests auf `hel_js_paths()` umgestellt; 9 neue Anti-Regress-Tests in `test_hel_frontend_separation.py`. **0 neue Regressions** im Test-Suite (Baseline 45/3392 → 45/3401, +9 Pass, 0 new Fail). Nala-Blueprint (Commit `f4f689e`) 1:1 reproduziert — bestaetigt: das Pattern (AST-Extraktion + Shared-Helper + Anti-Regress-Tests) funktioniert auf weitere Inline-Monolithen anwendbar. Hel-Spezifika: vier JS-Bloecke statt drei (FOUC im `<head>` zusaetzlich); Zeilen-Limit 4.500 (Hel hat ~2.683 Python-Admin-Routen, viel mehr als Nala mit ~750). Lone-Surrogate-Detail beim Extrahieren als neue Lesson dokumentiert. Vorherige Aktualisierung: Frontend-Separation (2026-05-19) — `nala.py` 8.528 → 1.381 Zeilen. Inline-CSS (~2.587) + 3 JS-Bloecke (~4.557) extrahiert nach `zerberus/static/css/nala.css` und `zerberus/static/js/nala-*.js`. NALA_HTML referenziert die Statics via `<link>` / `<script src>`. 28 Source-Audit-Tests via gemeinsamem Helper `zerberus/tests/_nala_source.py` migriert; 4 `node --check`-Tests auf Static-Files umgestellt; 8 neue Anti-Regress-Tests in `test_frontend_separation.py`. **0 Regressions** im Test-Suite (Baseline 44 → 42 Failures, 108 Errors → 108 Errors, alle pre-existing). `design_export/` Workaround entfernt + `.gitignore`-Eintrag geloescht. Vorherige Aktualisierung: Claude-Design-Export (2026-05-19) — Frontend-Analyse + `design_export/` erstellt. Kein Code-Patch. Davor: B-074 (2026-05-19) — Phase 5b Prio 1 ERLEDIGT. Zwei Blocks: (A) Phase-5b-Zieltabelle in `MARATHON_WORKFLOW_ZERBERUS.md` um Prio-Spalte erweitert mit Chris-Reihenfolge (1 Lessons-Cron, 2 Debugging Console, 3 Agent Observability, 4 Bugfix-Workflow + Test-Agents per Projekt, 5 Multi-Agent Orchestration, 6 Reasoning Modes, 7 Cost Transparency, 8 Multi-LLM Evaluation; Leitmotiv „Multi-Agent im Docker laeuft"). D-002 in `DECISIONS_ZERBERUS.md` auf ERLEDIGT gesetzt — APScheduler in-process gewinnt gegen separates Script (Begruendung Chris: Zerberus ist das Verwaltungszentrum, wenn Zerberus down ist sind auch die Projekte down die Lessons erzeugen). (B) B-074 Lessons-Cron implementiert: Neues Modul [`zerberus/modules/lessons_cron/runner.py`](zerberus/modules/lessons_cron/runner.py) mit Pure-Function-Schicht (`discover_lessons_files` + `aggregate_lessons_content` + `build_merge_messages` + `build_log_entry` + `count_patterns`) + Async-Wrapper `run_lessons_consolidation` + APScheduler-Hookup `register_lessons_cron_job` / `initialize_lessons_cron`. Pydantic-Klasse `LessonsCronConfig` in [`zerberus/core/config.py`](zerberus/core/config.py) (Default `enabled=False`, schedule_hour=4, schedule_minute=30). Lifespan-Hookup in [`zerberus/main.py`](zerberus/main.py) zwischen P214-Tasks-Init und Backup-GC. Config-Beispiel-Block in [`config.yaml.example`](config.yaml.example). 50 Unit-Tests in [`test_b074_lessons_cron.py`](zerberus/tests/test_b074_lessons_cron.py) gruen (Pure + Async-Mock-LLM + APScheduler-Mock + Source-Audit Lifespan + Settings-Integration + Modul-Struktur + Config-Example), B-073-Regression-Check 8/8 gruen. Logging-Tag `[LESSONS-074]`. Quell-`lessons_*.md` werden nur gelesen, nie modifiziert oder geloescht. Phase 5b Prio 2 (Debugging Console) ist der naechste logische Schritt.*

*Vorheriger Stand: B-decisions-init (2026-05-19) — Planungs- + Haushalts-Session. Drei Sub-Blocks: (A) WhatsApp-Modul (B-073) vollstaendig entfernt — `zerberus/modules/whatsapp/` geloescht, HITL-Protected-Channel-Set in `orchestrator.py` auf `{"telegram"}` reduziert, Kommentare in `legacy.py`/`orchestrator.py` bereinigt, `static/index.html`-Listing + `config.yaml.example`-Block + `requirements.txt`-twilio-Dep entfernt; 8 Source-Audit-Tests in [`test_b073_whatsapp_removed.py`](zerberus/tests/test_b073_whatsapp_removed.py) gruen. (B) Neue Workflow-Datei `DECISIONS_ZERBERUS.md` im Repo-Root angelegt mit drei initialen Eintraegen (D-001 WhatsApp-Entfernung ERLEDIGT, D-002 Lessons-Cron-Architektur OFFEN, D-003 MANUELLE_TESTS_CHECKLISTE OFFEN). `MARATHON_WORKFLOW_ZERBERUS.md` Doku-Pflicht-Tabelle um Eintrag fuer DECISIONS_ZERBERUS.md erweitert (Pflege-Regel: neue D-IDs, Status OFFEN/ERLEDIGT/OBSOLET, Zeile nie loeschen). (C) Phase 5b Zieltabelle in `MARATHON_WORKFLOW_ZERBERUS.md` angelegt — 8 Power-Features als GEPLANT (Multi-LLM Evaluation, Bugfix-Workflow + Test-Agents per Projekt, Multi-Agent Orchestration, Debugging Console, Reasoning Modes, Cost Transparency, Agent Observability, Lessons-Cron B-074); Reihenfolge offen, Chris priorisiert spaeter. Lessons-Cron (B-074) als blockiert durch D-002 markiert. Keine Phase-5b-Implementierung in dieser Session — nur Vorbereitung.*

*Vorheriger Stand: B-aufraeumen (2026-05-18) — Doku-Catchup-Session ohne Code-Aenderung. Zehn Marathon-Auftraege aus dem Zeitraum 2026-05-16 bis 2026-05-17 (B-061/P-umzug, B-072, B-mjolnir-fix, B-global-lessons, B-mjolnir-fix-2, B-mjolnir-multisession, B-status-snapshot, B-status-snapshot-Sync-Diagnose, B-db-merge, B-db-finale) retrospektiv in SUPERVISOR_ZERBERUS.md + docs/PROJEKTDOKUMENTATION.md eingetragen — der letzte regulaer dokumentierte Patch war P-mjolnir-workflow (2026-05-15), die Mjoelnir-Round-Trip-Pflicht hat die SUPERVISOR/PROJEKTDOKU-Pflege in den nachfolgenden Sessions teilweise verdraengt. Plus Stempel-Updates in `BACKLOG_ZERBERUS.md`-Header (Patch 182 → P217), `Ratatoskr/README.md`-Schlusszeile (Patch 100 → P217 + Phase 5a/5c abgeschlossen), `docs/huginn_kennt_zerberus.md`-Stand-Anker (P-mjolnir-workflow → P217 mit Hinweis auf nachfolgende B-Auftraege). Plus vier Status-Klaerungen aus dem 2026-05-17-Status-Snapshot: WhatsApp-Modul existiert im Code, BACKLOG-Eintrag fuer Entfernung neu angelegt; Llama Guard 3 8B war Konzept-Idee ohne Code-Spur, dokumentiert; Lessons-Cron-Job nicht eingerichtet, BACKLOG-Eintrag bestaetigt; `DECISIONS_ZERBERUS.md` nicht im Repo, als Workflow-Frage in `mjolnir.md` parkiert. Kein Code-Patch, kein Test-Restart, keine Architektur-Aenderung — reine Doku-Konsistenz.*

*Vorheriger Stand: P-mjolnir-workflow (2026-05-15) — Doku-only Workflow-Patch: `ZERBERUS_MARATHON_WORKFLOW.md` um drei Mjoelnir-Konventionen erweitert. Mjoelnir (Hammerfall) ist das Remote-Control-Panel fuer Coda-Sessions — kann per Knopfdruck Sessions starten, Feature-Requests einreichen, Session-Zusammenfassungen abrufen. Drei Aenderungen ohne Code-Pfad: (1) Session-Zyklus von 8 auf 12 Schritte erweitert — Schritt 0 prueft `FEATURE_REQUEST_ZERBERUS.md`-Existenz (Vorrang vor allem), Schritt 1 liest `mjolnir.md` aus Vorgaenger-Session ein (danach loeschen), Schritt 9 schreibt `mjolnir.md` (5-10 Zeilen Session-Zusammenfassung fuer den ZUSAMMENFASSUNG-Button), Schritt 10 benennt erledigten Feature-Request um zu `FEATURE_REQUEST_ZERBERUS_ERLEDIGT.md`. Plus Standard-Mjoelnir-Prompt zitiert als Block-Quote. (2) Doku-Pflicht-Tabelle um zwei Zeilen erweitert (mjolnir.md + FEATURE_REQUEST_ZERBERUS.md mit Format-/Umbenennungs-Regeln). (3) Neue Sektion „Mjoelnir-Konventionen" zwischen „Stopp-Regeln" und „Doku-Pflicht" — drei Sub-Abschnitte: mjolnir.md (Session-Zusammenfassung), FEATURE_REQUEST_ZERBERUS.md (priorisierter Arbeitsauftrag), und Rationale fuer separate Dateien statt MARATHON_WORKFLOW.md zu editieren (Feature-Requests kommen via Mjoelnir-API ohne manuelles Editieren). Akzeptanzkriterien: alter Prompt „Lies HANDOVER.md und ZERBERUS_MARATHON_WORKFLOW.md. Mach weiter." funktioniert weiterhin — die Feature-Request-Pruefung ist im Session-Zyklus verankert (Schritt 0), nicht prompt-abhaengig. Keine anderen Dateien beruehrt, keine Code-Aenderung, kein neuer Test, kein Server-Restart noetig. Davor: P-debt-14 (2026-05-10) — Test-Fix fuer `test_fenrir_mega_patch.py::TestPacemakerStress::test_f_pace_01_master_toggle_stable` (Schulden #3 GELOEST, Test-only). Test skipte konsistent seit Patch 99 (H-F01), weil das Hel-UI-Akkordeon durch eine Tab-Leiste ersetzt wurde und `.hel-section[data-tab='sysctl']:not(.active)` `display: none` ist. Fix: `activateTab('sysctl')`-Click VOR dem Master-Toggle-Click, analog zum gruen laufenden `test_f_pace_02`. Davor: P-debt-13 (2026-05-10) — Stand-Anker-Regex `extract_current_patch` in `tools/sync_huginn_rag.py` um Buchstaben-Suffix-Patches erweitert (Schulden #12 GELOEST, ein-Zeilen-Wartungs-Patch). Regex `(P\d{3,4})\b` matchte vorher keine `P-UI-*`/`P-debt-*`/`P-feature-audit-*`-Patches — drei `TestDocSourceAudit`-Tests blieben seit dem ersten P-UI-*-Stand-Anker (2026-05-09 nach P-UI-Hel-Split) rot. Fix: Regex auf `(P[\w-]+)` erweitert + `extract_current_patch` upper-cased nur das fuehrende `P` (Original-Case-Praeservierung fuer Integration-Lookups). Plus 12 neue Source-Audit-Tests in `test_p210_huginn_rag_sync.py`. **Schulden #12 GELOEST.** Davor: P-feature-audit-mt (2026-05-10, Manuelle-Tests-Tab in `feature_audit_checklist.html`), P-start-stable (2026-05-10, Reload+Force-Shutdown in `start_stable.bat`), P-feature-audit (2026-05-10, Initialwurf `scripts/feature_audit.py`), P-debt-12 (2026-05-10, Leitregel-Anker in DESIGN.md, Schulden #11 GELOEST), Schulden #9 (Hel-Splitscreen) GELOEST nach Chris' visueller Verifikation am 2026-05-10. 92/92 in `test_p210_huginn_rag_sync.py` gruen, 121/121 in der huginn-doc/design-system-Smoke-Suite gruen, 4 pre-existing Worktree-Drift-Failures wegen `faiss`-Modul (Schulden #6, im Hauptrepo gruen). Phase 5a + Phase 5c bleiben VOLLSTAENDIG ABGESCHLOSSEN. Letzter UI-Bugfix-Patch P-UI-Hel-Split. Letzter Backend-Bugfix-Patch P217. Letzter Code-Patch mit UI-Mechanik P-UI-11.*

---

## FEATURE_REQUEST_ZERBERUS Nachtrag (2026-05-21, Session 2/2) — Gist-Sync-Aufholarbeit + Migration-Plan + Login-Polish

**Auftrag.** FEATURE_REQUEST_ZERBERUS.md Nachtrag (Pflicht vor allem anderen): (a) Audit warum Schritt 12 in den letzten 4 Sessions uebersprungen wurde, (b) beide Projekt-Gists auf aktuellen Stand patchen, (c) Verstoss als Lesson dokumentieren, (d) dann weiter mit Komponenten-Migration. **STATUS: IN_ARBEIT** — Nachtrag-Pflichten alle erledigt, plus erster Komponenten-Schritt nach neuem Plan. FEATURE_REQUEST bleibt liegen (9 Sub-Sessions stehen aus).

**Phase 1 — Gist-Sync-Aufholarbeit.** Audit `git log` zeigt: B-074 (`e080f90`, 2026-05-19), Nala-Frontend-Separation (`f4f689e`, 2026-05-19), Hel-Frontend-Separation (`cec3a76`, 2026-05-20), FEATURE_REQUEST Teil-Lieferung (`b7dcd08`, 2026-05-21) haben alle Schritt 12 (Gist-Sync) uebersprungen. mjolnir.md war jeweils geschrieben, HANDOVER war jeweils da — aber der Gist-Sync als gleichrangiger Schritt am Session-Ende fiel unter den Tisch. Klassisches **Faulheits-Catch #2 Derivat**. Lesson am Top von [`lessons_ZERBERUS.md`](lessons_ZERBERUS.md) + `C:\Users\chris\Python\Claude\GLOBAL_LESSONS.md` zementiert (Pipe-Format-Header + Anlass-Block + Loesung + generalisierbare Lesson). Claude-Repo Commit `9da822a` + push erfolgt. **Zerberus-Gist** `7f5af9dd878a6a9d664f062976b27ae8` per GitHub-API-PATCH aktualisiert (HTTP 200): `STATUS.md` (874 B, aus mjolnir-Header generiert) + `HANDOVER.md` (16030 B) + `MJOLNIR.md` (12012 B) + `LESSONS.md` (316303 B) + `REPO_INDEX.md` (18737 B). **Claude-KB-Gist** `48b997e53ff331eeefef53c810ee7331` per PATCH (HTTP 200): `GLOBAL_LESSONS.md` (16920 B). Token via `git credential fill`. PATCH-Skripte inline (`/tmp/gist_patch.py` + `/tmp/gist_kb_patch.py`) — bestehende `gist_publisher.py` kann nur `create`, `update_gist` nur als Library-Function. **Nach Doku-Update muss der Zerberus-Gist erneut gepatcht werden** — HANDOVER + MJOLNIR + LESSONS sind nach Session-Ende um diese Session erweitert.

**Phase 2 — Cleanup.** `docs/Zerberus.zip` (176 KB, **tracked**) per `git rm` entfernt — Inhalt liegt als `nala-kintsugi.css` + `nala-kintsugi-prototype.js` + `zerberus/static/assets/` schon im Repo, ZIP war Redundanz. `.zerberus_zip_tmp/` (727 KB, gitignored) per `rm -rf` entfernt — Arbeitskopie wird nicht mehr gebraucht.

**Phase 3 — Multi-Session-Migration-Plan.** [`docs/NALA_KINTSUGI_MIGRATION_PLAN.md`](docs/NALA_KINTSUGI_MIGRATION_PLAN.md) **NEU** — zerlegt die noch ausstehende Komponenten-Migration in 10 fokussierte Sub-Sessions (N+1 Login, N+2 Header, N+3 Sidebar, N+4 Chat-List+Bubbles, N+5 Reasoning/Code-Card/Output-Card/HitL, N+6 Input-Bar, N+7 Scroll-Nav, N+8 Project-View, N+9 Settings-Modal, N+10 Final mit Mobile-Layout + Cleanup + Doku-Refresh). Strategie: **Class-Annex statt Class-Rename** — neue Style-Properties via existierende Selektoren ergaenzen, keine Class-Renames. **Heilige Anker** enumeriert (`#chatMessages`, `#text-input`, `#sendBtn`, `#micBtn`, `#imageBtn`, `#login-username`, alle Patch-119–147-Anker, etc.) — JS und Tests referenzieren sie, NICHT umbenennen. Pro Session: Baseline-Test-Diff vor + nach Migration, **0 neue Regressions** als Akzeptanz-Kriterium. Dieser Plan ist ab jetzt das verbindliche Multi-Session-Roadmap-Dokument fuer die FEATURE_REQUEST-Rest-Erledigung.

**Phase 4 — Erster Komponenten-Schritt (Plan-Session N+1): Login-Polish.** [`zerberus/static/css/nala.css`](zerberus/static/css/nala.css) — 2 Selektoren additiv ergaenzt:

- `#login-screen h2`: `font-family: var(--zb-font-display, inherit); font-weight: 700; letter-spacing: -0.01em;` — Playfair Display greift, wenn `nala-design.css` (Foundation-Layer) geladen ist. Fallback auf `inherit` falls Foundation fehlt (Hel unbehelligt).
- `.login-hint`: `font-family: var(--zb-font-mono, inherit); letter-spacing: 0.22em; text-transform: uppercase;` — JetBrains-Mono-Caps-Stil, gleicher Fallback-Mechanismus.

**Sicherheits-Check vorher:** `grep -rln "login-screen h2|login-hint"` → keine Test-Anker, sichere Aenderung. Variable-Fallback-Pattern → bei fehlender Foundation faellt CSS auf `inherit` zurueck → Hel unbeeinflusst. nala.css-File-Size 2608 Zeilen (unter 3000-Limit aus Lesson „Mega-Files in main.py vermeiden").

**Test-Ergebnis.** `test_frontend_separation.py` → 8/8 PASSED (Source-Audit, keine neuen Anker-Verletzungen). `test_login.py` + `test_design.py` + `test_kintsugi.py` + `test_nala_source.py` → 108/108 PASSED. **0 neue Regressions.** Voll-Suite nicht erneut gefahren — Baseline 45/3401/108 ist bekannt aus Vor-Session, eine 2-Selektor-additive-CSS-Aenderung ohne Backend-Beruehrung kann die Failure-Liste nicht aendern.

**Stolperstein.** Pflichtaufgabe „Audit warum Schritt 12 uebersprungen wurde" zeigt: **vier** Sessions in Folge haben den Sync ausgelassen. Es war kein Einzelfall, sondern ein systemisches Muster. Loesung in der Lesson: Schritt 12 ist **gleichrangig** mit mjolnir.md und HANDOVER, NICHT optional. Beim ersten Anzeichen von Token-Knappheit lieber HANDOVER+mjolnir+Gist-Sync zuerst, dann Code-Cleanup zuletzt — Gist-Sync ist Kommunikations-Brücke zur Supervisor-Instanz, ohne ihn arbeitet Supervisor mit veraltetem Stand.

**Edits dieser Session.**

- **`lessons_ZERBERUS.md`** — Neue Lesson am Top: „Gist-Sync (Schritt 12) ist gleichrangige Session-End-Pflicht — Faulheits-Catch #2 Derivat (Gist-Drift 2026-05-21)".
- **`zerberus/static/css/nala.css`** — Login-Polish: 2 additive Selektoren (Playfair-h2 + Mono-Caps-Hint).
- **`docs/NALA_KINTSUGI_MIGRATION_PLAN.md`** — NEU, Multi-Session-Plan (10 Sub-Sessions, Heilige Anker, Strategie, Akzeptanz-Kriterien).
- **`docs/Zerberus.zip`** — GELOESCHT (war tracked, 176 KB; Inhalt liegt entpackt im Repo).
- **`.zerberus_zip_tmp/`** — GELOESCHT (war gitignored, 727 KB Arbeitskopie).
- **`HANDOVER_ZERBERUS.md`** + **`mjolnir.md`** + **`SUPERVISOR_ZERBERUS.md`** (diese Datei) + **`docs/PROJEKTDOKUMENTATION.md`** + **`README.md`** + **`docs/huginn_kennt_zerberus.md`** + Spiegel — Stand-Anker auf 2026-05-21 Session 2/2 aktualisiert.
- **`C:\Users\chris\Python\Claude\GLOBAL_LESSONS.md`** — Lesson global zementiert, Claude-Repo Commit `9da822a` + push erfolgt.
- **Zerberus-Gist `7f5af9dd...`** — per PATCH (HTTP 200).
- **Claude-KB-Gist `48b997e5...`** — per PATCH (HTTP 200).

**FEATURE_REQUEST_ZERBERUS.md bleibt liegen.** 9 Sub-Sessions stehen noch aus (N+2 bis N+10). Plan-Dokument ist die verbindliche Roadmap.

---

## FEATURE_REQUEST_ZERBERUS Teil-Lieferung (2026-05-20 → 2026-05-21) — Multi-Session, STATUS: IN_ARBEIT

Feature-Request-Patch (gemischt Bugfix + Feature). FEATURE_REQUEST von Chris ueber Mjoelnir mit fuenf Punkten plus Hauptauftrag „Nala Design-Integration aus `docs/Zerberus.zip`". Priorisierung Chris: Design → F-01 → B-01 → B-02/B-03 → B-04. **STATUS: IN_ARBEIT** — alle vier Bugs (B-01, B-02, B-03, B-04) und das Feature F-01 sind komplett umgesetzt; **nur die Kintsugi-Komponent-Migration ist vertagt** (Foundation-Layer geliefert, siehe unten). FEATURE_REQUEST_ZERBERUS.md wird **nicht** umbenannt (Multi-Session-Regel).

**Design-Integration — strategische Entscheidung.** Der Claude-Design-Export aus `docs/Zerberus.zip` (Kintsugi Void/Gold/Ember-Palette) ist ein UI-only-Prototyp. Er hat **kein** Backend-Wiring: kein SSE, kein Voice-Aufnahme, kein jsPDF-Export, kein Pacemaker, kein OpenRouter-Profile-Header-Handling. Blindes Mergen wuerde alle bestehenden Nala-Pfade brechen. Entscheidung: **Foundation-Layer-only** in dieser Session — Kintsugi-Tokens isoliert in neuer Datei [`zerberus/static/css/nala-design.css`](zerberus/static/css/nala-design.css), die NACH `nala.css` geladen wird (Cascade-Override nur auf Nala). KRITISCH: die neuen Tokens duerfen NICHT in `shared-design.css` — Hel nutzt die alten `--zb-primary`-Variablen und wuerde sofort brechen. Prototyp-Files bleiben als Read-Only-Referenz im Repo (`nala-kintsugi.css`, `nala-kintsugi-prototype.js` — **nicht** geladen, nicht gelinkt). Komponent-Migration (neue Bubble-Styles, Recorder-UI, Modal-Layout) folgt in Anschluss-Session — dann mit voller Backend-Pflege.

**F-01 — Nala Bild-Upload-Button.** Vision-Modell (`qwen/qwen2.5-vl-7b-instruct` via OpenRouter) war konfiguriert, aber im UI fehlte der Upload-Button. Implementiert: (a) HTML in [`zerberus/app/routers/nala.py`](zerberus/app/routers/nala.py) (verstecktes `<input type="file" id="image-upload" accept="image/*">` + sichtbarer `<button class="image-btn" id="imageBtn">📷</button>`). (b) Neuer Endpoint `POST /nala/image` mit `UploadFile = File(...)` + `prompt: str = Form("")` — wichtig: bei `UploadFile` brauchen weitere non-file-Form-Felder zwingend `Form(...)`, sonst ignoriert FastAPI sie. Endpoint liest Bytes, prueft `<= max_image_size_mb * 1024 * 1024` (Default 10 MB, Config-Key `max_image_size_mb` mit `max_image_mb`-Fallback), ruft `analyze_image(image_data, prompt, model, max_bytes)` aus [`zerberus/utils/vision.py`](zerberus/utils/vision.py), liefert `{content, error, latency_ms, model}`. (c) JS-Handler in [`zerberus/static/js/nala-main.js`](zerberus/static/js/nala-main.js) nach `toggleRecording`: Listener auf `change`, baut `FormData`, schickt `fetch('/nala/image', { method:'POST', headers: profileHeaders(), body: formData })`, handhabt 401 (Logout), 413 (Toast „Bild zu gross"), generische Fehler — waehrend Request: Button-Disable + `.processing`-Klasse + Pulse-Animation. (d) CSS in [`zerberus/static/css/nala.css`](zerberus/static/css/nala.css): `.image-btn` zur `.send-btn, .mic-btn`-Selector-Group hinzugefuegt (50×50 px = Touch-Target ≥44px), Gold-Background (`var(--color-gold)`), Hover/Active-State, `:disabled`/`.processing` mit `pulseGold 1.2s infinite`-Animation.

**B-04 — Huginn 409 Conflict / Zombie-Prozesse Dauerfix.** Wurzel: Telegram-Bot-API erlaubt nur **einen** aktiven `getUpdates`-Caller pro Bot-Token. Mehrere Python-Prozesse (z.B. nach Hard-Kill oder Crash mit verwaisten Workern) bekommen alle HTTP 409. Shutdown-Hook in `main.py` ist korrekt, hilft aber nicht bei `SIGKILL`/Crash. Loesung: **PID-Lock** + cross-platform Liveness-Check. In [`zerberus/modules/telegram/bot.py`](zerberus/modules/telegram/bot.py): (a) Modul-Konstanten `PID_LOCK_FILE = Path("data/huginn_polling.pid")`, `_LAST_POLL_409`, `_POLL_409_BACKOFF = 15.0`, `_poll_409_warning_emitted`. (b) `_pid_is_alive(pid)` — Windows via `ctypes.windll.kernel32.OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, False, pid)` + `GetExitCodeProcess(h, &exit_code)` + Vergleich gegen `STILL_ACTIVE=259`; Unix via `os.kill(pid, 0)`. (c) `acquire_polling_lock()` liest existierende PID, prueft Liveness, ueberschreibt nur wenn alter Caller tot ist; `release_polling_lock()` loescht die Datei beim Shutdown. (d) `get_updates()` erkennt HTTP 409 → setzt `_LAST_POLL_409 = True`, gibt `[]` zurueck. (e) `long_polling_loop` ist von `try/finally: release_polling_lock()` umschlossen; 409-Detection triggert 15s-Backoff mit Single-WARN (kein Logging-Flood). In [`zerberus/modules/telegram/router.py`](zerberus/modules/telegram/router.py): `startup_huginn()` ruft `acquire_polling_lock()` vor `asyncio.create_task(long_polling_loop(...))`. Liefert es `False`, wird das Polling in dieser Instanz nicht gestartet (WARN-Log). [`.gitignore`](.gitignore): `data/huginn_polling.pid` eingetragen — der Lock-File-Pfad ist Repo-Working-State, nicht versionierbar.

**B-03 — Hel LLM-Tab Sortierung nach Name.** In [`zerberus/static/js/hel-main.js`](zerberus/static/js/hel-main.js) lag eine tote Code-Pfad vor: bei `_currentSort === 'name'` lief der **gleiche** Vergleicher wie beim Preis-Pfad (`parseFloat(a.pricing?.prompt) - parseFloat(b.pricing?.prompt)`). Fix: Name-Pfad bekommt `nameA.localeCompare(nameB, 'de', { sensitivity: 'base' })` — case-insensitive, Umlaute korrekt einsortiert. „Patch 147"-Comment-Anker wiederhergestellt, weil `test_default_sort_nach_preis` ihn als Strukturmarker prueft (sonst neue Regression).

**B-02 — Hel LLM-Tab Kontostandanzeige.** Wurzel: OpenRouter Pre-paid-Accounts liefern `data.limit == null` bei `/auth/key`, womit die alte `float(limit_usd) - float(usage)`-Berechnung in [`zerberus/app/routers/hel.py::get_balance`](zerberus/app/routers/hel.py) `None` zurueckgibt und das Frontend (`hel-main.js`) bei der Anzeige in einen `TypeError: Cannot read property 'toFixed' of null` lief. Backend-Fix: bei `limit_usd is None` zusaetzlicher Call auf `https://openrouter.ai/api/v1/credits` — der liefert `total_credits` + `total_usage`, die als neue `limit_usd`/`usage` einsetzen. Frontend-Fix: USD-Suffix wird nur gerendert wenn `curBalance != null && !Number.isNaN(curBalance)`, EUR-String nur wenn `balanceEur != null && !Number.isNaN(balanceEur)` — sonst Fallback „Kontostand: nicht verfuegbar".
...
**Stolpersteine.** (1) `Form(...)`-Pflicht bei `UploadFile`: ohne `Form()` werden non-file-Felder in Multipart-Requests ignoriert. (2) Config-Key-Verwechslung `max_image_mb` vs. `max_image_size_mb` — aktuelle Codebase nutzt letzteres; defensiver Fallback eingebaut. (3) `PROJECTS`-Env-Var Konflikt mit `pydantic-settings`: Chris hat global `PROJECTS=Zerberus:...,Mjolnir:...` gesetzt, das kollidiert mit `Settings.projects`. Workaround dokumentiert: `PROJECTS= python -m pytest ...` (leere Var explizit setzen).

**Multi-Session-Status.** [`mjolnir.md`](mjolnir.md): **STATUS: IN_ARBEIT** mit Offene-Punkte-Liste fuer Folge-Session (Design-Komponent-Migration, B-01, B-02, Cleanup `.zerberus_zip_tmp/`). [`HANDOVER_ZERBERUS.md`](HANDOVER_ZERBERUS.md) als Top-Eintrag mit Zwischenstand. FEATURE_REQUEST_ZERBERUS.md **nicht** umbenannt — Auftrag laeuft weiter.
...
- **`SUPERVISOR_ZERBERUS.md`** (diese Datei) — Header + neue Sektion.
- **`HANDOVER_ZERBERUS.md`** — Diese Session als Top-Eintrag (Zwischenstand).
- **`mjolnir.md`** — **STATUS: IN_ARBEIT** + Offene-Punkte fuer Folge-Session.
- **`lessons_ZERBERUS.md`** — 2 neue Lessons (Foundation-Layer-Trennung, PID-Lock).
- **`docs/huginn_kennt_zerberus.md`** (+ Spiegel in `docs/RAG Testdokumente/`) — Stand-Anker.
- **`FEATURE_REQUEST_ZERBERUS.md`** — **NICHT** umbenannt (Multi-Session).

---

## Hel-Frontend-Separation (2026-05-20) — hel.py inline-Monolith aufgeloest

Architektur-Patch. FEATURE_REQUEST von Chris ueber Mjoelnir: Inline-Frontend aus [`zerberus/app/routers/hel.py`](zerberus/app/routers/hel.py) (6.576 Zeilen) extrahieren nach dem **gleichen Pattern wie die Nala-Separation** (Commit `f4f689e`). Blueprint, Lessons + Shared-Helper-Pattern aus Nala komplett wiederverwendbar — Schaetzung 1 Session, in einer Session abgeschlossen.

**Architektur-Entscheidung.** Variante B (Static Files mit `<link rel="stylesheet">` + `<script src>`), identisch zur Nala-Wahl. ADMIN_HTML bleibt als Python-Triple-Quoted-String erhalten; der Static-Mount in [`zerberus/main.py`](zerberus/main.py) liefert die Files via HTTP; Browser-Caching, IDE-Tooling, Source-Audit-Tests via Shared-Helper bleiben funktional.

**Extraktion.** Inline-CSS (~420 Zeilen) extrahiert nach [`zerberus/static/css/hel.css`](zerberus/static/css/hel.css). **Vier** JS-Bloecke (~2.558 Zeilen) zerlegt nach Funktion: [`zerberus/static/js/hel-fouc.js`](zerberus/static/js/hel-fouc.js) (FOUC-Prevent — laed `localStorage` Font-Size + Active-Tab im `<head>` vor dem ersten Paint; Hel-spezifisch, kein Pendant in Nala), [`zerberus/static/js/hel-main.js`](zerberus/static/js/hel-main.js) (Haupt-App-Logik — der grosse Block am Datei-Ende, ~2.470 Zeilen), [`zerberus/static/js/hel-gpu-toast.js`](zerberus/static/js/hel-gpu-toast.js) (GPU-Queue-Polling-Toast aus P211), [`zerberus/static/js/hel-pwa-sw.js`](zerberus/static/js/hel-pwa-sw.js) (Service-Worker-Registrierung). ADMIN_HTML referenziert die Statics via fuenf `<link>`/`<script src>`-Tags an den jeweils passenden Stellen (CSS + FOUC im `<head>`, Main + GPU-Toast + PWA-SW vor `</body>`). Endergebnis: `hel.py` 6.576 → **3.604 Zeilen** (Reduktion: 45,2 %).

**Lone-Surrogate-Detail.** Beim Schreiben von `hel-main.js` crashte das mit `UnicodeEncodeError` an Position 63574-63575. Ursache: `\ud83d\udcc4` (📄-Emoji) im Datei-Manager-JS als lone Surrogate-Paar. Fix: `body.encode('utf-8', errors='replace').decode('utf-8', errors='replace')` vor `Path.write_text()` — mirror der `_sanitize_unicode()`-Semantik aus dem Hel-Endpoint. Damit ist die geschriebene Datei pixel-identisch zu dem was der Browser sieht. Neue Lesson in [`lessons_ZERBERUS.md`](lessons_ZERBERUS.md): „Lone Surrogates bei UTF-8-Datei-Schreibung aus Triple-Quoted-Source".

**Test-Anpassung.** 15 lokale `hel_src`-Fixtures + 2 Regex-basierte JS-Integrity-Tests + 1 Hel-Endpoint-Smoke-Test inventarisiert. Shared-Helper [`zerberus/tests/_hel_source.py`](zerberus/tests/_hel_source.py) analog zu `_nala_source.py` gebaut: `read_combined_hel_source()` konkateniert `hel.py` + alle 5 Statics; `combine_admin_html_with_statics(admin_html)` haengt die Statics an `hel.ADMIN_HTML` fuer Smoke-Tests; `hel_js_paths()` liefert die 4 JS-Files in Load-Order fuer `node --check`. 16 Test-Files migriert: test_cost_display, test_design_system, test_dialect_ui, test_hel_kleinigkeiten, test_memory_dashboard, test_metrics_cleanup, test_model_dropdown_unified, test_pacemaker_controls, test_patch169_bugsweep, test_patch170_hel_kosmetik, test_p_ui_hel_split, test_projects_ui, test_pwa, test_huginn_config_endpoint, test_p213_pre_3_reindex_atomic, test_p203b_hel_js_integrity, test_p205_hel_rag_toast, test_p211_gpu_queue, test_p214_pre_2_scheduler_ui. Plus 2 `node --check`-Tests auf `hel_js_paths()` umgestellt (test_p203b, test_p205, test_p211 — die JS-Integrity-Klassen darin).

**Anti-Regress-Tests.** Neuer File [`zerberus/tests/test_hel_frontend_separation.py`](zerberus/tests/test_hel_frontend_separation.py) mit **9 Tests in 3 Klassen**: `TestNoInlineFrontend` (kein `<style>`-Tag, kein Inline-`<script>`-Body, alle 5 Static-Refs vorhanden), `TestStaticFilesPresent` (alle 5 Files exist + non-empty + Marker), `TestHelPyShrunk` (< 4.500 Zeilen — grosszuegig gewaehlt wegen ~2.683 Python-Admin-Routen). Wenn jemand den Monolithen versehentlich wiederherstellt: rot.

**Test-Ergebnis.** **0 neue Regressions.** Baseline (vor Hel-Separation, per `git stash` gemessen): 45 Failures, 108 Errors, 3.392 Passes. Nach kompletter Migration: **45 Failures, 108 Errors, 3.401 Passes** — die +9 Passes sind die neuen Anti-Regress-Tests. Per direkter Liste-Vergleich: **null neue Failures.** Pytest-Voll-Lauf dauerte 2:15 min.

**Lesson.** Das Nala-Pattern reproduziert sich. Wer einen weiteren Inline-Monolith-Router findet, wendet das gleiche Vorgehen wieder an — AST-Extraktion + Shared-Helper + Anti-Regress-Tests + Migration der Source-Audit-Fixtures. Hel-spezifische Anpassungen waren minimal: vier JS-Bloecke statt drei (FOUC im `<head>` zusaetzlich), Anti-Regress-Zeilen-Limit 4.500 statt 3.000 (Profil-abhaengig: bei Python-leichten Routern strenger, bei Python-schweren grosszuegiger).

**Edits dieser Session.**

- **`zerberus/app/routers/hel.py`** — 6.576 → 3.604 Zeilen (Inline-CSS + 4 JS-Bloecke raus, `<link>` + 4 `<script src>` rein).
- **`zerberus/static/css/hel.css`** — NEU, ~420 Zeilen (extrahiertes Inline-CSS).
- **`zerberus/static/js/hel-fouc.js`** — NEU (FOUC-Prevent im `<head>`).
- **`zerberus/static/js/hel-main.js`** — NEU (Haupt-App-Logik).
- **`zerberus/static/js/hel-gpu-toast.js`** — NEU (GPU-Queue-Toast).
- **`zerberus/static/js/hel-pwa-sw.js`** — NEU (Service-Worker-Registrierung).
- **`zerberus/tests/_hel_source.py`** — NEU, Shared-Helper.
- **`zerberus/tests/test_hel_frontend_separation.py`** — NEU, 9 Anti-Regress-Tests.
- **16 bestehende Test-Files** — auf Helper migriert.
- **`SUPERVISOR_ZERBERUS.md`** (diese Datei) — Header + neue Sektion.
- **`HANDOVER_ZERBERUS.md`** — Diese Session als Top-Eintrag.
- **`mjolnir.md`** — STATUS: FERTIG, alle Phasen dokumentiert.
- **`lessons_ZERBERUS.md`** — Lesson erweitert (Nala-Blueprint bestaetigt) + neue Lesson (lone Surrogates).
- **`FEATURE_REQUEST_ZERBERUS.md`** → `FEATURE_REQUEST_ZERBERUS_ERLEDIGT.md`.

---

## Frontend-Separation (2026-05-19) — nala.py inline-Monolith aufgeloest

Architektur-Patch. FEATURE_REQUEST von Chris ueber Mjoelnir: Inline-Frontend aus [`zerberus/app/routers/nala.py`](zerberus/app/routers/nala.py) (8.528 Zeilen) extrahieren und in Static-Files auslagern. Nach Claude-Design-Export-Session (Vorgaenger) war klar: `design_export/` war nur ein temporaerer Arbeitsordner — die richtige Loesung ist die Aufloesung des Monolithen am Quelltext.

**Architektur-Entscheidung.** Variante B (Static Files mit `<link rel="stylesheet">` + `<script src>`) statt Variante A (Jinja2-Templates). Begruendung: NALA_HTML bleibt als Python-Triple-Quoted-String erhalten (Refs auf Python-Variable `WORKSPACE_*`-Konstanten im Template bleiben funktionsfaehig); der Static-Mount in [`zerberus/main.py`](zerberus/main.py) (`app.mount("/static", StaticFiles(directory="zerberus/static"))`, Z. 428) ist bereits etabliert und liefert die Files via HTTP; Browser-Caching kann pro Asset greifen (vorher: kompletter Re-Download des 8.5k-Zeilen-Strings pro Request); IDE-Tooling (Linting, Syntax-Highlight, Prettier) funktioniert nativ auf `.css`/`.js`-Dateien; und die Tests koennen die Source-Markers weiterhin via direktem Dateilesen pruefen.

**Extraktion.** Inline-CSS (~2.587 Zeilen) extrahiert nach [`zerberus/static/css/nala.css`](zerberus/static/css/nala.css). Drei JS-Bloecke (~4.557 Zeilen) zerlegt nach Bedeutung: [`zerberus/static/js/nala-blackbug.js`](zerberus/static/js/nala-blackbug.js) (Black-Bug-Sanitizer IIFE — muss frueh laufen, deshalb im `<head>`), [`zerberus/static/js/nala-main.js`](zerberus/static/js/nala-main.js) (Haupt-App-Logik — der grosse Block am Datei-Ende), [`zerberus/static/js/nala-pwa-sw.js`](zerberus/static/js/nala-pwa-sw.js) (Service-Worker-Registrierung — eigene Datei aus PWA-Hygiene). NALA_HTML referenziert die Statics via `<link rel="stylesheet" href="/static/css/nala.css">` und drei `<script src="/static/js/nala-*.js">`-Tags an den jeweils passenden Stellen (Blackbug im `<head>`, Main + PWA-SW vor `</body>`). Endergebnis: `nala.py` 8.528 → **1.381 Zeilen** (Reduktion: 83,8 %).

**Python-String-Literale → JS-Bytes.** Extrahiertes JS enthielt Backslash-Sequenzen die als Python-String-Literale geschrieben waren (z.B. `\\s\\S` im Source, was Python decodiert zu `\s\S` als JS-Regex-Literal). Loesung: `ast.literal_eval()` auf den extrahierten Triple-Quoted-String — Python decodiert die Escapes korrekt, dann in die `.js`-Datei schreiben. Sonst waren die Regex-Literale alle falsch (`\\s` statt `\s`) und Node bricht beim `--check` ab.

**Test-Anpassung (Core-Risiko).** 28 Source-Audit-Tests lasen `nala.py` direkt via `Path(...).read_text()` und pruefen Substrings (Marker-Tags, CSS-Klassen, JS-Funktions-Namen). Nach der Extraktion finden sie ihre Marker nicht mehr in `nala.py`. **Loesung:** Gemeinsamer Helper [`zerberus/tests/_nala_source.py`](zerberus/tests/_nala_source.py) mit drei Hilfsfunktionen — `read_combined_nala_source()` konkateniert `nala.py` + alle 4 Statics zu einem Riesen-String, `combine_endpoint_body_with_statics(body)` haengt die Statics an einen Endpoint-Response-Body fuer Smoke-Tests, `nala_js_paths()` gibt eine Liste der drei JS-Files fuer `node --check`-Tests zurueck. 12 Test-Files migriert (test_p206 / test_p207 / test_p208 / test_p209 / test_p_ui_3 / test_p_ui_4 / test_p_ui_5 / test_p_ui_polish_2 / test_p_ui_11 / test_p_ui_hel_split / test_prosody_consent / test_settings_umbau / test_p203d3). Drei Patterns: (1) Fixture-Body-Swap (`return Path(NALA_PY).read_text()` → `return read_combined_nala_source()`); (2) Endpoint+Statics fuer Smoke-Tests (`html = body + ...` → `html = combine_endpoint_body_with_statics(body)`); (3) Direktes Static-File-Read fuer Marker-spezifische Tests (`Path(NALA_PY).read_text()` → `NALA_CSS_PATH.read_text()` o.ae.). Plus: 4 `node --check`-Tests umgestellt — vorher extrahierten sie `<script>` aus dem nala-HTML-String und schrieben in Tempfile, jetzt iterieren sie ueber `nala_js_paths()` und rufen Node direkt auf den `.js`-Files auf.

**Anti-Regress-Tests.** Neuer File [`zerberus/tests/test_frontend_separation.py`](zerberus/tests/test_frontend_separation.py) mit **8 Tests in 3 Klassen**: `TestStaticDateienVorhanden` (4 Files exist + non-empty), `TestNalaHtmlReferenziertStatics` (4 — `<link>`-Tag fuer CSS, drei `<script src>`-Tags), `TestKeineInlineFrontendBlockeMehr` (3 — `<style>`-Tag fehlt, IIFE-Marker nicht inline, PWA-Marker nicht inline). Wenn jemand den Monolithen versehentlich wiederherstellt: rot.

**Test-Ergebnis.** **0 Regressions.** Baseline (vor Extraktion): 44 Failures, 108 Errors, vorher per `git stash` + pytest gemessen. Nach kompletter Migration: **42 Failures, 108 Errors** — die 2 weniger Failures sind sogar zwei Tests die vorher schon kaputt waren und nebenbei mit der Helper-Migration mitrepariert wurden. Per `comm -13 baseline.txt current.txt` verifiziert: **null neue Failures.** Pytest-Voll-Lauf dauerte 4:27 min, kein Crash.

**Phase 5 Cleanup.** `design_export/` (15 Dateien) per `rm -rf` geloescht — der Arbeitsordner ist obsolet, das Frontend liegt jetzt sauber in `zerberus/static/`. `.gitignore`-Eintrag `design_export/` (plus Kommentar `# Claude Design Arbeitsordner`) entfernt — der Ordner existiert nicht mehr.

**Stolpersteine retrospektiv.** (1) `_find_sidebar_html_block` in [`test_settings_umbau.py`](zerberus/tests/test_settings_umbau.py) nutzte `"Patch 142 (B-013)"` als Anker, dieser Marker erscheint im kombinierten Source-String aber zuerst in einem HTML-Kommentar im Tab „System" (vor der Sidebar), nicht in der Sidebar selbst — Fix: Anker auf `'<div class="sidebar" id="sidebar">'` umgestellt (eindeutig). (2) `test_p209::test_nala_html_passes_node_check` extrahierte vorher `<script>` aus HTML und war damit baseline-broken (kein passender `<script>`-Tag mehr da nach Refactor); umgestellt auf `nala_js_paths()`-Iteration — von rot zu gruen. (3) Python-String-Escapes vs JS-Regex-Literale (siehe oben) — der Fall war ueberhaupt nicht offensichtlich, der `node --check`-Run hat ihn aufgedeckt.

**Edits dieser Session.**

- **`zerberus/app/routers/nala.py`** — 8.528 → 1.381 Zeilen (Inline-CSS + 3 JS-Bloecke raus, `<link>` + 3 `<script src>` rein).
- **`zerberus/static/css/nala.css`** — NEU, 2.587 Zeilen (extrahiertes Inline-CSS).
- **`zerberus/static/js/nala-blackbug.js`** — NEU (Sanitizer-IIFE).
- **`zerberus/static/js/nala-main.js`** — NEU (Haupt-App-Logik).
- **`zerberus/static/js/nala-pwa-sw.js`** — NEU (Service-Worker-Registrierung).
- **`zerberus/tests/_nala_source.py`** — NEU, gemeinsamer Helper (`read_combined_nala_source`, `combine_endpoint_body_with_statics`, `nala_js_paths`).
- **`zerberus/tests/test_frontend_separation.py`** — NEU, 8 Anti-Regress-Tests in 3 Klassen.
- **12 bestehende Test-Files** — auf Helper migriert (test_p206 / p207 / p208 / p209 / p_ui_3 / p_ui_4 / p_ui_5 / p_ui_polish_2 / p_ui_11 / p_ui_hel_split / prosody_consent / settings_umbau / p203d3).
- **`design_export/`** — GELOESCHT (15 Dateien, war Arbeitsordner).
- **`.gitignore`** — `design_export/`-Eintrag + Kommentarzeile geloescht.
- **`SUPERVISOR_ZERBERUS.md`** (diese Datei) — Header + neue Sektion.
- **`docs/PROJEKTDOKUMENTATION.md`** — Bibel-Eintrag.
...
**Was diese Session NICHT gemacht hat.** Hel-Modul ([`zerberus/app/routers/hel.py`](zerberus/app/routers/hel.py)) bleibt vorerst inline — Phase 4 des FEATURE_REQUEST war als optional markiert und der Time-Budget-Spielraum war durch die 12-Datei-Test-Migration aufgebraucht. BACKLOG-Eintrag fuer hel-Separation als Nachfolge-Patch ergaenzt. Keine Funktionalitaet im Nala-Frontend veraendert — reine Architektur-Aenderung; das UI sieht identisch aus, Tests bestaetigen Verhalten unveraendert.

**Anti-Verstoss-Check (OBERSTES GEBOT).** Null Terminal-Befehle an Chris delegiert — Extraktion, Tests, Doku, Commits, Push, Sync alles autonom.

---

## Claude-Design-Export (2026-05-19) — Nala-Frontend-Analyse + design_export/ erstellt

Analyse-Session (kein Code-Patch). FEATURE_REQUEST von Chris via Supervisor: Nala-Frontend-Dateien fuer Claude Design identifizieren + ggf. Extraktion-Ordner anlegen.

**Befund Schritt 1 (Analyse).** Nala-Frontend ist monolithisch — HTML/CSS/JS inline in [`zerberus/app/routers/nala.py`](zerberus/app/routers/nala.py) (8.528 Zeilen). CSS-Block: lines 121-2709 (~2.587 Zeilen). JS-Blöcke: lines 2710-2901 + 3402-7763 (~4.551 Zeilen). Externe Dateien: [`zerberus/static/css/shared-design.css`](zerberus/static/css/shared-design.css) (116 Zeilen Design-Tokens — CSS Custom Properties `--color-primary`/`--color-gold`/`--zb-space-*` etc.), [`zerberus/static/css/style.css`](zerberus/static/css/style.css) (164 Zeilen Legacy), [`zerberus/static/js/audio.js`](zerberus/static/js/audio.js) (22 Zeilen). Assets: PWA-Icons + favicon + Branding-Bild unter `zerberus/static/`.

**Befund Schritt 2 (Empfehlung).** Kein einzelner Subordner — Frontend verteilt auf `app/routers/` (Python-Inline) + `static/` (externe Dateien). Schritt 3 noetig.

**Ergebnis Schritt 3 (design_export/).** Lokaler Arbeitsordner `design_export/` im Repo-Root erstellt (via `.gitignore` ausgeschlossen — kein Deployment-Artefakt): `styles/nala.css` (extrahiertes Inline-CSS, 2.587 Zeilen), `styles/shared-design.css` + `style.css` (Kopie), `scripts/nala.js` (extrahiertes Inline-JS, 4.551 Zeilen), `scripts/audio.js` (Kopie), `assets/` (6 Bild-/Icon-Dateien), `reference/nala.py` + `hel.py` + `DESIGN.md` + `NalaMockup.jsx` (Kopien, keine Moves), `design_export/README.md`. `.gitignore` um `design_export/` erweitert.

**Fuer Chris.** Empfohlener Einstiegspunkt in Claude Design: `design_export/styles/shared-design.css` (Design-Tokens) + `design_export/styles/nala.css` (Haupt-Styles) + `design_export/reference/DESIGN.md`. Hinweis: `design_export/` ist via `.gitignore` ausgeschlossen — fuer GitHub-Import in Claude Design muss Chris den Ordner temporaer committen (`git add -f design_export/ && git commit`) oder Dateien direkt hochladen.

**Edits dieser Session.**
- **`design_export/`** — NEU (Arbeitsordner, 15 Dateien, via `.gitignore` ausgeschlossen).
- **`.gitignore`** — `design_export/`-Eintrag ergaenzt.
- **`SUPERVISOR_ZERBERUS.md`** (diese Datei) — Header + neue Sektion.
- **`HANDOVER_ZERBERUS.md`** — neu geschrieben.
- **`mjolnir.md`** — STATUS=FERTIG.
- **`FEATURE_REQUEST_ZERBERUS.md`** — umbenannt zu `FEATURE_REQUEST_ZERBERUS_ERLEDIGT.md`.

---

## B-074 (2026-05-19) — Phase 5b Prio 1: Lessons-Cron implementiert

Code-Patch (Mini-Modul + Lifespan-Hookup + Tests + Doku). Erster Patch der Phase 5b. Folgt direkt auf B-decisions-init (gleicher Tag) — die Reihenfolge ist via Supervisor-Session-Auftrag entschieden, D-002 ist mit dieser Session ERLEDIGT.

**Architektur-Entscheidung D-002 (Chris).** APScheduler-Job im Zerberus-Prozess gewinnt gegen separates Python-Script unter Windows Task Scheduler. Begruendung: Zerberus ist das Verwaltungszentrum — wenn Zerberus down ist, sind auch die Projekte down die Lessons erzeugen. Keine externe Abhaengigkeit noetig. Misfire-Grace 1h (analog Overnight-Job P57). Zeitpunkt 04:30 Europe/Berlin (steht so in der Memory-Spec; weit nach dem letzten Patch-Push, vor dem ersten Frueh-Login).

**Block A — Phase-5b-Zieltabelle priorisiert.** Prio-Spalte in `MARATHON_WORKFLOW_ZERBERUS.md` eingefuegt. Reihenfolge der acht Features: (1) Lessons-Cron (B-074, Warmup, kleiner Patch, D-002 entschieden); (2) Debugging Console (Sichtbarkeit VOR Orchestrierung — ohne Logs/Token/RAG-Treffer kein Debugging); (3) Agent Observability; (4) Bugfix-Workflow + Test-Agents per Projekt (Loki/Fenrir/Vidar pro Nala-Projekt isoliert — Grundlage fuer Multi-Agent); (5) Multi-Agent Orchestration (Kernziel — alles davor baut darauf hin); (6) Reasoning Modes; (7) Cost Transparency; (8) Multi-LLM Evaluation. Leitmotiv: „Multi-Agent im Docker laeuft". D-002 in `DECISIONS_ZERBERUS.md` auf ERLEDIGT.

**Block B — B-074 Lessons-Cron Code.** Neues Modul [`zerberus/modules/lessons_cron/`](zerberus/modules/lessons_cron/) mit `__init__.py` (Helper-Modul-Marker, kein `router.py` — analog `modules/scheduler/` wird nicht vom Auto-Loader erfasst) und `runner.py`. **Pure-Function-Schicht** (vollstaendig testbar ohne Mocks): `discover_lessons_files(scan_paths)` findet alle `lessons_*.md` rekursiv (skipt nicht-existente Pfade, dedupliziert), `aggregate_lessons_content(files)` liest + concatieniert mit Pfad-Headern (skipt unlesbare Dateien fail-quiet), `build_merge_messages(content)` baut 2-Element-LLM-Messages-Array (System-Prompt fordert universelle Lessons-Extraktion ohne projektspezifische Patch-IDs/Dateipfade, User-Prompt liefert aggregierten Content), `build_log_entry(*, timestamp, files_count, patterns_count, error=None)` formatierter Markdown-Listenpunkt fuer `CONSOLIDATION_LOG.md` (Newlines im Error-Feld werden zu Spaces, eine Zeile pro Lauf), `count_patterns(llm_output)` grobe Audit-Schaetzung via Bullet-/Header-Zaehlung. **I/O-Schicht:** `write_output(path, content)` atomic via `tempfile + os.replace`-Pattern (Parent-Dir on-demand), `append_log(path, entry)` haengt eine Zeile an (Parent-Dir on-demand). **Async-Schicht:** `run_lessons_consolidation(cfg, *, llm_service=None)` ist der High-Level-Run — discover → aggregate → LLM-Call (Lazy-Import + neue `LLMService`-Instanz, session_id `lessons-cron-074`) → write_output → append_log. Returns Dict `{files_count, patterns_count, status, error}` ohne Crash auf jeden Pfad-Fehler. **APScheduler-Hookup:** `register_lessons_cron_job(scheduler, cfg)` haengt eine Closure `_run` als Cron-Job ein (id=`lessons-cron-074`, `replace_existing=True`, `misfire_grace_time=3600`) — die Closure liest Settings live ueber `get_settings()` damit Config-Reloads greifen. `initialize_lessons_cron(scheduler)` ist der Lifespan-Einstieg. **Pydantic-Klasse `LessonsCronConfig`** in [`zerberus/core/config.py`](zerberus/core/config.py) zwischen `PrevalidationConfig` und `ProfileConfig` plus Settings-Feld `lessons_cron: LessonsCronConfig = LessonsCronConfig()`. Felder: `enabled: bool = False`, `schedule_hour: int = 4`, `schedule_minute: int = 30`, `scan_paths: List[str] = []`, `output_path: str = ""`, `consolidation_log: str = "CONSOLIDATION_LOG.md"`. Default `enabled=False` ist gewollt — Job schreibt in andere Repos (`Bmad82/Claude/lessons/`), das muss explizit aktiviert werden. **Lifespan-Hookup** in [`zerberus/main.py`](zerberus/main.py) zwischen P214-Tasks-Init und P213-pre-4-Backup-GC: prueft `settings.lessons_cron.enabled`, ruft `initialize_lessons_cron(_scheduler)` im try/except. Startup-Log-Item `Lessons-Cron` mit `ok` (`HH:MM (N Pfad(e))`) / `skip` (`deaktiviert (lessons_cron.enabled=false)`) / `skip` (`kein APScheduler`) / `fail` (`Job-Registrierung fehlgeschlagen`). **Config-Beispiel-Block** in [`config.yaml.example`](config.yaml.example) vor JWT-Sektion mit 6 Feldern + Erlaeuterungs-Kommentar.

**Tests.** Neuer File [`zerberus/tests/test_b074_lessons_cron.py`](zerberus/tests/test_b074_lessons_cron.py) mit **50 Unit-Tests in 13 Klassen**: `TestDiscoverLessonsFiles` (7 — rekursiv, leer, nicht-existent, leer-string, Datei-statt-Dir, Dedup, Konstante), `TestAggregateLessonsContent` (4 — Header-Format, leere Liste, leere Datei, unlesbare Datei), `TestBuildMergeMessages` (4 — Anzahl, GLOBAL_LESSONS-Mention, Payload-Pass-Through, System-Prompt-Konstante), `TestBuildLogEntry` (3 — ok-Pfad, error-Pfad, Newline-Sanitize), `TestCountPatterns` (4 — Bullets+Header, leer, kein Match, eingerueckt), `TestWriteOutput` (3 — atomic neu, ueberschreibt, kein tmp uebrig), `TestAppendLog` (2 — neu, append), `TestRunLessonsConsolidation` (5 — Happy-Path mit Mock-LLM, leere Files, LLM-Fehler-String, LLM-Crash, Quell-Datei unveraendert), `TestRegisterLessonsCronJob` (5 — Happy, disabled, kein Scheduler, APScheduler-Crash, Konstante), `TestSettingsIntegration` (3 — Pydantic-Default, Settings-Feld, init ohne Scheduler), `TestMainPyHookupSourceAudit` (4 — Import, Aufruf, Log-Item, enabled-Pruefung), `TestModulStruktur` (4 — Verzeichnis, runner.py, kein router.py, __init__.py), `TestConfigYamlExample` (2 — Sektion vorhanden, enabled-Default-false). **50/50 PASSED** in 1.00s. Plus B-073-Regression-Check **8/8 PASSED** (kombiniert in 6.84s, ein FutureWarning aus huggingface_hub unverwandt).

**Edits dieser Session.**

- **`zerberus/modules/lessons_cron/__init__.py`** — NEU, Helper-Modul-Marker.
- **`zerberus/modules/lessons_cron/runner.py`** — NEU, Pure + Async + APScheduler-Hookup.
- **`zerberus/core/config.py`** — `LessonsCronConfig`-Klasse + Settings-Feld `lessons_cron`.
- **`zerberus/main.py`** — Lifespan-Hookup mit `_log_item("Lessons-Cron", ...)`.
- **`config.yaml.example`** — 9-Zeilen-Block plus 8 Kommentar-Zeilen, vor JWT-Sektion.
- **`zerberus/tests/test_b074_lessons_cron.py`** — NEU, 50 Tests in 13 Klassen.
- **`MARATHON_WORKFLOW_ZERBERUS.md`** — Phase-5b-Tabelle umgebaut: Prio-Spalte, Reihenfolge eingetragen, B-074 auf ERLEDIGT, D-002-Vermerk im Tabellen-Header.
- **`DECISIONS_ZERBERUS.md`** — D-002 auf ERLEDIGT mit Datum + Begruendung.
...
- **`SUPERVISOR_ZERBERUS.md`** (diese Datei) — Header umgeschrieben, neue Sektion B-074 als erste Inhaltssektion.
- **`docs/PROJEKTDOKUMENTATION.md`** — Bibel-Eintrag dieses Patches angehaengt.
- **`HANDOVER_ZERBERUS.md`** — neu geschrieben.
- **`mjolnir.md`** — STATUS=FERTIG fuer B-074.
- **`FEATURE_REQUEST_ZERBERUS.md`** — umbenannt zu `FEATURE_REQUEST_ZERBERUS_ERLEDIGT.md` mit Audit-Anhang.

**Was diese Session NICHT gemacht hat (wie im FEATURE_REQUEST vorgegeben).**

- Keine Features ab Phase-5b-Prio-2 (Debugging Console).
- `MANUELLE_TESTS_CHECKLISTE.md` nicht angefasst (D-003).
- Rosa/Heimdall nicht angefasst.

**Anti-Verstoss-Check (OBERSTES GEBOT).** Null Terminal-Befehle an Chris delegiert — Modul-Anlage, Tests, Doku, Commits, Push, Sync alles autonom.

---

## B-decisions-init (2026-05-19) — Phase 5b Vorbereitung + Haushalts-Auftrag (3 Blocks)

Planungs- + Haushalts-Session, kein Notfall. Drei Sub-Blocks plus Doku-Pflege.

**Block A — WhatsApp-Modul entfernen (B-073 ERLEDIGT).** Filesystem-Check ergab: Modul existiert noch (`zerberus/modules/whatsapp/router.py` mit Twilio-Webhook, `__init__.py`). `git log --all -- zerberus/modules/whatsapp/` zeigte nur `b4a95d3 Initial commit` — Modul war von Anfang an im Repo, nie produktiv genutzt, Twilio-Dep auch nie installiert (`TWILIO_AVAILABLE=False` der Default-Branch). Entfernt: (1) Verzeichnis `zerberus/modules/whatsapp/` komplett geloescht. (2) [`zerberus/app/routers/orchestrator.py`](zerberus/app/routers/orchestrator.py): `_HITL_PROTECTED_CHANNELS` von `{"telegram", "whatsapp"}` auf `{"telegram"}` reduziert, Kommentar Z. 156-159 aktualisiert, Kommentar Z. 449 aktualisiert. (3) [`zerberus/app/routers/legacy.py`](zerberus/app/routers/legacy.py): Kommentar Z. 453-454 erwaehnt nur noch Telegram. (4) [`zerberus/static/index.html`](zerberus/static/index.html): `<li>whatsapp – WhatsApp Bridge</li>` aus Modul-Listing entfernt. (5) [`config.yaml.example`](config.yaml.example): `whatsapp:`-Sektion (5 Zeilen) komplett raus. (6) [`requirements.txt`](requirements.txt): `twilio==8.10.0` plus Kommentar raus. **Tests.** Neuer File [`zerberus/tests/test_b073_whatsapp_removed.py`](zerberus/tests/test_b073_whatsapp_removed.py) mit **8 Source-Audit-Tests in 6 Klassen** (TestWhatsappModulEntfernt 2 / TestHitlProtectedChannelsOhneWhatsapp 2 / TestFrontendListingOhneWhatsapp 1 / TestConfigBeispielOhneWhatsapp 1 / TestRequirementsOhneTwilio 1 / TestLegacyKommentarBereinigt 1). 8/8 gruen lokal. BACKLOG-Item B-073 auf ERLEDIGT gesetzt. DECISIONS-Eintrag D-001 auf ERLEDIGT gesetzt.

**Block B — DECISIONS_ZERBERUS.md angelegt.** Neue Datei im Repo-Root mit drei initialen Eintraegen: D-001 (WhatsApp-Modul entfernen — ERLEDIGT via Block A), D-002 (Lessons-Cron: APScheduler-im-Prozess vs. separates Python-Script unter Windows Task Scheduler — OFFEN, Chris entscheidet), D-003 (MANUELLE_TESTS_CHECKLISTE.md noch aktuell — OFFEN, Chris arbeitet ab). Pflege-Regel dokumentiert: neue Entscheidungs-Fragen bekommen fortlaufende D-ID, Status-Werte sind OFFEN / ERLEDIGT — <wie/wann> / OBSOLET — <warum>, Zeile nie loeschen damit Audit-Trail bleibt. Doku-Pflicht-Tabelle in `MARATHON_WORKFLOW_ZERBERUS.md` um Pflicht-Eintrag fuer DECISIONS_ZERBERUS.md erweitert.

**Block C — Phase 5b Zieltabelle angelegt.** Neue Sektion `## Phase 5b — Power-Features` in `MARATHON_WORKFLOW_ZERBERUS.md` direkt nach Phase 5c eingefuegt. Acht Features als GEPLANT: Multi-LLM Evaluation, Bugfix-Workflow + Test-Agents per Projekt, Multi-Agent Orchestration, Debugging Console, Reasoning Modes (effort-score-Pre-Call + User-Override), Cost Transparency, Agent Observability, Lessons-Cron (B-074, blockiert durch D-002). Reihenfolge offen — Chris priorisiert per Mjölnir-Kanal oder naechster Supervisor-Session. Erfuellt damit das Erfolgskriterium aus dem FEATURE_REQUEST: "Nach dieser Session: Phase 5b steht, Reihenfolge fehlt noch."

**Block D (Lessons-Cron B-074) — nicht implementiert.** Wie im FEATURE_REQUEST gefordert: D-002 ist Architektur-Frage, wartet auf Chris. Nur dokumentiert (Block B), nicht implementiert.

**Edits dieser Session.** (1) `zerberus/modules/whatsapp/` geloescht; (2) `zerberus/app/routers/orchestrator.py` zwei Kommentar-Bloecke + HITL-Set; (3) `zerberus/app/routers/legacy.py` ein Kommentar-Block; (4) `zerberus/static/index.html` ein `<li>`-Element; (5) `config.yaml.example` 5-Zeilen-Block; (6) `requirements.txt` zwei Zeilen; (7) `zerberus/tests/test_b073_whatsapp_removed.py` neu (8 Tests); (8) `DECISIONS_ZERBERUS.md` neu im Repo-Root; (9) `MARATHON_WORKFLOW_ZERBERUS.md` Doku-Pflicht-Tabelle plus neue Phase-5b-Sektion; (10) `BACKLOG_ZERBERUS.md` Header-Stempel + B-073 auf ERLEDIGT + B-074 mit D-002-Verweis; (11) `SUPERVISOR_ZERBERUS.md` (diese Datei) Header + neue Session-Sektion; (12) `docs/PROJEKTDOKUMENTATION.md` Bibel-Eintrag; (13) `HANDOVER_ZERBERUS.md` neu geschrieben; (14) `mjolnir.md` STATUS=FERTIG; (15) `FEATURE_REQUEST_ZERBERUS.md` → `FEATURE_REQUEST_ZERBERUS_ERLEDIGT.md` umbenannt.

**Anti-Verstoss-Check (OBERSTES GEBOT).** Null Terminal-Befehle an Chris delegiert — Modul-Loeschung, Tests, Doku, Commits, Push, Sync alles autonom.

---

## Doku-Catchup (B-aufraeumen, 2026-05-18) — zehn Marathon-Auftraege retrospektiv eingetragen

Zwischen P-mjolnir-workflow (2026-05-15) und der heutigen Session ist die SUPERVISOR/PROJEKTDOKU-Pflege haengen geblieben, waehrend zehn Mjoelnir-Auftraege im Zeitraum 2026-05-16 bis 2026-05-17 sauber durchgegangen sind (mit HANDOVER + mjolnir.md + `_ERLEDIGT.md`-Audit + Commits). Diese Sektion holt die Eintraege retrospektiv nach, chronologisch absteigend (neuester zuerst). Die Marathon-Workflow-Konvention bleibt: Detail-Doku in PROJEKTDOKUMENTATION.md (Bibel-Format), strategische Zusammenfassung hier.

### B-db-finale (2026-05-17) — Letzte 3 nala_memory*.db in bunker_memory.db konsolidiert

Folge-Auftrag zu B-db-merge. Die drei verbleibenden lesbaren `.db` aus `legacy/alte Datenbanken/` (`nala_memory.db`, `nala_memory (2).db`, `nala_memory (3).db`, alle aus Whisper-Weichen-Lineage) wurden mit Schema-Mismatch-Handling in die produktive `bunker_memory.db` gemerged. **+208 interactions** (12855 → 13060), **+158 message_metrics** (12638 → 12799 incl. 3 Live-Service-Inserts), **+0 costs**, **+2 protokoll** (neue Tabelle in Ziel-DB angelegt mit src-CREATE-SQL). Alle 7 Phasen (0_init bis 6_sync) in einer Session, 8/8 Akzeptanzkriterien erfuellt. Pre-Finale-Backup `legacy/alte Datenbanken/bunker_memory_pre_finale_2026-05-17T21-41.db` (11.91 MB, online-safe via `sqlite3.Connection.backup()`). FK-Konsistenz: 35 Pre-Orphans = 35 Post-Orphans (keine neuen). Tool-Erweiterungen in `tools/db_merge_*`: `unreadable`-Klassifikation, `merge_generic_table()` fuer unbekannte Tabellen, CLI-Parametrisierung im Smoke-Test. Commits Zerberus `b09c6df` (Merge-Run) + `a04dc8d` (State-File final). Lessons-Sync ueber `sync_repos.ps1`. **Schemata.** Schema-Mismatch-Handling-Pattern (NULL-Fill bei fehlenden Spalten, dynamisches CREATE-TABLE bei NEW-Tabellen, Skip bei Type-Drift) ist jetzt im Toolset wiederverwendbar fuer kuenftige Snapshot-Migrationen. **Anti-Verstoss-Check (OBERSTES GEBOT):** Null Terminal-Befehle an Chris delegiert — Service-Stopp, Backup, Merge, Smoke-Test, Service-Restart, Sync alles autonom.

### B-db-merge (2026-05-17) — bunker_memory.db Konsolidierungs-Vorgaenger

Mjoelnir-Auftrag (Prio 1, Housekeeping, mehrphasig). Alle drei Quell-DBs aus `legacy/alte Datenbanken/` (`bunker_memory_backup_patch92.db`, `bunker_memory_backup_patch113a.db`, alt-uralt `bunker_memory.db` mit 252 Rows) deduplikativ in die produktive Root-DB gemerged. **+287 interactions, +35 message_metrics, +0 costs.** Tools `tools/db_merge_*.py` (inspect/backup/analyze/run/smoketest) erstmals etabliert. Pre-Merge-Backup `legacy/alte Datenbanken/bunker_memory_pre_merge_2026-05-17T17-49.db` (11.55 MB). Service-Stopp via `taskkill /F /T /PID 63068`, BEGIN IMMEDIATE-Transaktionen mit COMMIT, Service-Restart via uvicorn aus bash mit `unset PROJECTS` (Lesson — pydantic-settings versuchte die Bash-Mehrzeil-`PROJECTS`-Env-Var als Pydantic-Feld zu parsen, wurde mit `SettingsError` quittiert). Schema-Drift erkannt (patch92 fehlt `profile_key`, legacy-uralt fehlt `session_id`+`profile_name`+`profile_key`), Dedup-Strategie pro Tabelle finalisiert (PK-Match + Content-Hash). FK-Konsistenz: 35 pre-existing Orphans in message_metrics, keine neuen durch Merge. Commits Zerberus `5e3e17c` (Merge-Run) + `21a6a66` (State-File final). 8/8 Akzeptanzkriterien erfuellt. Out-of-scope dokumentiert: `nala_memory*.db` damals wegen Schema-Mismatch konservativ uebersprungen → Folge-Auftrag B-db-finale.

### B-status-snapshot Sync-Diagnose (2026-05-17) — Fall E, keine Reparatur noetig

Folge-Auftrag zu B-status-snapshot. Supervisor (`web_fetch`) hatte gemeldet `STATUS_AKTUELL.md` sei nicht auf Ratatoskr-main, obwohl mjolnir.md "Push erfolgreich" sagte. Diagnose entlang der Sechs-Punkte-Liste aus dem FEATURE_REQUEST: Datei existiert lokal (16294 Byte), Commit `67e50e5` als HEAD, working tree clean, 0 unpushed, `origin/main` byte-genau identisch mit HEAD. `git ls-tree origin/main --name-only` zeigt `STATUS_AKTUELL.md` enthalten. `verify_sync.ps1` gruen auf Ratatoskr. HTTP-HEAD-Check auf `https://github.com/Bmad82/Ratatoskr/blob/main/STATUS_AKTUELL.md` → 200 OK; `raw.githubusercontent.com/.../STATUS_AKTUELL.md` → 200 OK, Content-Length 16294 Byte (matched lokale Dateigroesse). Zerberus-Repo-Cross-Check: Datei nicht in `C:\Users\chris\Python\Zerberus`, nicht in Alt-Pfad — korrekt nur in Ratatoskr. **Fall E (kein Sync-Fehler).** Interpretation: Caching-Delay zwischen `git push` und GitHub-Web-UI/`web_fetch`-CDN. Kein Patch-Fehler. Commit Zerberus `f0173dc` (.gitignore-Erweiterung fuer `legacy/alte Datenbanken/`) + `a51c62e` (HANDOVER-Nachtrag mit Commit-Hashes). **Lesson.** Supervisor-`web_fetch` kann gecached/veraltet sein — bei "Datei fehlt"-Befunden erst die drei Pflicht-Checks (`git ls-tree origin/main`, `verify_sync.ps1`, `Invoke-WebRequest -Method Head`) durchziehen statt blind zu re-pushen.

### B-status-snapshot (2026-05-17) — STATUS_AKTUELL.md fuer Supervisor erstellt

Mjoelnir-Auftrag — Coda erstellt einen kompakten Status-Snapshot fuer die Supervisor-Instanz. Output `Ratatoskr/STATUS_AKTUELL.md` (220 Zeilen, 9 Sektionen): Patch- und Branch-Stand, Phase-5a-Ist-Stand, Test-Stand, VRAM-Tetris (RTX 3060 12 GB), Phase-5b-Planung, offene Bugs/Dauerbrenner, Erledigt seit letztem Supervisor-Update, Inkonsistenzen-Liste (6 Punkte, die genau diese Aufraeum-Session aufgegriffen hat), Doku-Sync-Status. Spec-konform Single-Source-of-Truth, keine Duplikate aus HANDOVER. Ratatoskr-Commit `67e50e5` auf main gepusht. Zerberus-Commit `cd1073b` (HANDOVER-Update). **Inkonsistenzen-Befund aus dem Snapshot** (Anlass fuer B-aufraeumen): SUPERVISOR + PROJEKTDOKU stehen auf P-mjolnir-workflow (2026-05-15) waehrend HANDOVER + Git auf B-mjolnir-multisession (2026-05-17); BACKLOG-Header auf Patch 182 (2026-04-30); Ratatoskr README-Schlusszeile auf Patch 100 (2026-04-19); `huginn_kennt_zerberus.md` Stand-Anker auf P-mjolnir-workflow; WhatsApp-Modul-Status nicht verifiziert; Llama-Guard-Historie nicht verifiziert; Lessons-Cron-Status nicht verifiziert; `DECISIONS.md`-Datei nicht im Repo.

### B-mjolnir-multisession (2026-05-17) — STATUS-Header + QUEUED-Pattern + dreiphasiger Selbsttest

Doku-only Patch in `MARATHON_WORKFLOW_ZERBERUS.md`. Drei Aenderungen, alle ohne Code-Pfad. **STATUS-Header als erster Block in mjolnir.md** — Pflichtformat `**STATUS:** FERTIG | IN_ARBEIT | BLOCKIERT` + `**AUFTRAG:** <Kurzname>` + `**FORTSCHRITT:** <X von Y Sessions>` + `**NÄCHSTE SESSION:** <was zuerst dran ist> | entfaellt (FERTIG)` + bei IN_ARBEIT/BLOCKIERT zusaetzlich `**WARNUNG:** KEINEN NEUEN FEATURE_REQUEST SCHICKEN — alter Auftrag noch in Bearbeitung`. Chris kann am Status auf einen Blick erkennen ob Geduld noetig ist. **QUEUED-Pattern** — wenn Chris einen neuen FEATURE_REQUEST schickt waehrend der alte noch laeuft (mjolnir.md STATUS = IN_ARBEIT, Auftrag-Kurzname unterschiedlich): neuen Request zu `FEATURE_REQUEST_ZERBERUS_QUEUED.md` umbenennen, alten aus mjolnir.md rekonstruieren, alten zuerst zu Ende bringen, dann QUEUED dran. Dadurch ist ueberschreibender Doppel-Submit harmlos. **Dreiphasiger Selbsttest** im Workflow als Pflicht verankert — Phase 1: Vor jedem Commit Lokal-Selbsttest; Phase 2: Pre-Push remote-vergleichbar; Phase 3: nach Push HTTP-HEAD auf GitHub raw. Commit Zerberus `7ba1262` + `20a5727`. **Lesson.** Multi-Session-Auftraege brauchen einen sichtbaren Status-Header an erster Stelle in mjolnir.md, sonst entstehen ueberschreibende Doppel-Submits. Status-im-Inhalt versteckt verlaesst sich auf Chris-Augen.

### B-mjolnir-fix-2 (2026-05-16/17) — mjolnir.md-Round-Trip verschaerft

Folge-Auftrag zu B-mjolnir-fix. Die Pflicht "mjolnir.md am Session-Ende ueberschreiben" stand schon im Workflow — aber sie war als if/else formuliert ("wenn FEATURE_REQUEST vorlag", "wenn Tokens uebrig"). B-mjolnir-fix-2 macht sie unbedingt: gleichrangig mit HANDOVER (Schritt 8) und Push (Schritt 12), AUSNAHMSLOS, am Ende JEDER Session — auch wenn kein FEATURE_REQUEST vorlag, auch wenn die Session nur ein Mini-Patch war, auch wenn das Token-Budget knapp ist. Plus Cleanup-Regel: wenn beim Session-Start keine mjolnir.md existiert, normaler Start (z.B. nach `git clone`); aber am Session-Ende MUSS sie da sein. Anti-Pattern aus B-072 zitiert (mjolnir.md damals NICHT ueberschrieben, weil `_ERLEDIGT.md`-Audit faelschlich als Ersatz interpretiert wurde). Klarstellung Single-Slot vs Audit-Log: mjolnir.md ist Single-Slot-State (genau eine aktuelle Session-Zusammenfassung), `_ERLEDIGT.md` ist Audit-Log (akkumuliert). Commit Zerberus `c56d66c` + `0e183d0`. **Lesson.** Round-Trip-Pflichten gehoeren als unbedingt formuliert ("AUSNAHMSLOS, AM ENDE JEDER SESSION"), nicht als if/else — sonst findet der naechste Faulheits-Drift einen Optional-Hinterausgang.

### B-global-lessons (2026-05-16) — 4 Marathon-Workflow-Lessons ins Claude-Repo

Vier universelle Lessons aus den Mai-Sessions (P-umzug + B-061 + B-072 + B-mjolnir-fix) in `Bmad82/Claude/GLOBAL_LESSONS.md` (universelle Lessons fuer alle Projekte im Marathon-Workflow) eingetragen. (1) **OBERSTES GEBOT** — Chris terminalisiert nichts was Coda kann; niemals git/pytest/pip/robocopy/npm-Befehle an Chris delegieren; Coda merged Branches SELBST + pusht SELBST vor Session-Ende; mjolnir.md enthaelt NUR was physisch unmoeglich ist; Verstoss = sofortige Korrektur. Backstop pro Projekt: Regel 0 in `CLAUDE_<PROJEKT>.md` plus erste Sektion in `lessons_<PROJEKT>.md`. (2) **mjolnir.md ist PFLICHT am Session-Ende** — Anlass B-072 (mjolnir.md damals nicht ueberschrieben). (3) **Worktree-Branches selbst auf main mergen** — Anlass B-061 (vorige Coda-Session hatte ungemergten Branch in mjolnir.md an Chris delegiert). Pattern: `git stash push` → `git merge --ff-only` → `git stash pop` → eigenen Worktree-Branch per `git rebase main` aktualisieren → ff-merge → push. (4) **Supervisor-Verhalten: Coda-Prompts statt Terminal-Befehle** — auch "nur ein Befehl" ist einer zu viel; die Frage ist immer "Kann Coda das selbst?", Antwort ist in 95 % der Faelle ja. Plus geplant nachgetragen: **Multi-Session-Status-Header fuer mjolnir.md** (2026-05-17, mit B-mjolnir-multisession). Commit Zerberus `63d3081` (Lessons-Sync ueber `sync_repos.ps1`). **Lesson.** Lessons die universell gueltig sind gehoeren ins Claude-Repo (`Bmad82/Claude`), projektspezifisch ins Zerberus-Repo (`lessons_ZERBERUS.md`). Universalitaet entscheidet sich nach "kann das jedes andere Marathon-Projekt brauchen?" — `OBERSTES GEBOT` ja, `unset PROJECTS vor uvicorn aus bash` nein.

### B-mjolnir-fix (2026-05-16) — mjolnir.md-Round-Trip-Pflicht im Workflow verankern

Erste Doku-Verankerung der mjolnir.md-Schreib-Pflicht in `MARATHON_WORKFLOW_ZERBERUS.md`. Anlass: B-072 (Huginn Voice-to-Whisper) war vollstaendig gelaufen (Code + Tests + Commit + `_ERLEDIGT.md`-Audit) — aber `mjolnir.md` wurde nicht ueberschrieben. Chris hat ueber Mjoelnir den ZUSAMMENFASSUNG-Button gedrueckt, gefetcht hat die alte P-umzug-Datei. Damit war fuer Chris nicht erkennbar ob B-072 fertig ist. Round-Trip kaputt. **Fix.** Sektion "Mjoelnir-Konventionen" in MARATHON_WORKFLOW.md erweitert um klare Schreib-Pflicht am Session-Ende. Plus Anti-Pattern-Block "Was passiert ohne mjolnir.md-Update". Plus klare Trennung Single-Slot vs Audit-Log. Commit Zerberus `88c55d9` + `185969a`. **Lesson.** Workflow-Konventionen ueberleben nur wenn sie als Ablauf-Pflicht im Session-Zyklus formuliert sind, nicht als Empfehlung am Sektions-Ende. Der nachfolgende Patch (B-mjolnir-fix-2) musste die Pflicht nochmal verschaerfen — Beweis dass die erste Formulierung zu weich war.

### B-072 (2026-05-16) — Huginn Voice→Whisper-Pipeline in Telegram

Sprachnachrichten in Telegram-DMs werden jetzt ueber den lokalen Whisper-Container transkribiert statt freundlich abzulehnen (vorher P182 Block 2 Option A — Unsupported-Media-Block). Backlog-Item B-072. **Aenderungen.** Neue Helper `download_telegram_file()` in [`zerberus/modules/telegram/bot.py`](zerberus/modules/telegram/bot.py) (Telegram-`getFile`-Endpoint + Download via Bot-Token-URL). Neuer Helper `_transcribe_voice_message()` in [`zerberus/modules/telegram/router.py`](zerberus/modules/telegram/router.py) (Whisper-Call via `zerberus.utils.whisper_client.transcribe` auf `localhost:8002`). Voice-Pfad in `_legacy_process_update` aus dem Unsupported-Media-Block ausgezweigt: Transkript wird in `message["text"]` injiziert und laeuft durch die normale Text-Pipeline (inkl. Persona, RAG, Guard, Memory). Whisper offline → eine freundliche Absage-Message ("Voice-Modul gerade nicht verfuegbar"), kein Crash. Leeres Transkript → eigene Silence-Fallback-Message ("Konnte nichts verstehen"). **Tests.** [`zerberus/tests/test_huginn_voice.py`](zerberus/tests/test_huginn_voice.py) mit 16 neuen Tests. Telegram/Huginn/Whisper-Suite gesamt 50/50 gruen. Commit Zerberus `c04772d`. **Lesson aus dieser Session (separat).** Die Session hat zwar Code + Tests + `_ERLEDIGT.md`-Audit geschrieben, aber `mjolnir.md` NICHT ueberschrieben — Chris fetchte die alte P-umzug-Datei. Folge: B-mjolnir-fix als naechster Patch.

### B-061 (2026-05-16) + P-umzug Teil 1+2 (2026-05-16) — Pfad-Audit + Umzug + OBERSTES GEBOT

Drei verzahnte Patches an einem Tag — B-061 hat zuerst die portable Sync-Script-Infrastruktur etabliert, P-umzug Teil 1 hat das OBERSTE GEBOT verankert (Anti-Faulheits-Backstop), Teil 2 hat den eigentlichen Zerberus-Umzug nach `C:\Users\chris\Python\Zerberus` durchgezogen. **B-061** (Commit `05ef9e8`): Pfad-Audit — null Code-Treffer in `.py` fuer hardcoded alte Pfade. `sync_repos.ps1` + `scripts/verify_sync.ps1` portabel via `$PSScriptRoot` + Env-Var-Overrides `ZERBERUS_RATATOSKR_PATH` / `ZERBERUS_CLAUDE_PATH` mit Geschwister-Detection-Fallback. Plus B-070-Easter-Egg (versteckt in [`zerberus/main.py`](zerberus/main.py) `create_app()`: `# make love ... not war — W.F. Weiher, Stanford AI Lab, 1967` — erstes dokumentiertes Software-Easter-Egg). **P-umzug Teil 1** (Commit `29b95bf`): OBERSTES GEBOT als Regel 0 in `CLAUDE_ZERBERUS.md` + erste Sektion in `lessons_ZERBERUS.md` verankert. Chris terminalisiert NICHTS was Coda kann, niemals git/pytest/pip/robocopy/npm-Befehle an Chris delegieren, mjolnir.md enthaelt NUR was physisch unmoeglich ist. **P-umzug Teil 2** (Commit `11307ab`): Zerberus-Repo physisch nach `C:\Users\chris\Python\Zerberus` umgezogen (robocopy, Venv-Neuaufbau, Service-Test). Ratatoskr und Claude-Repo bleiben am alten Ort (`C:\Users\chris\Python\Rosa\Nala_Rosa\Ratatoskr` + `C:\Users\chris\Python\Claude`). Server, Tests, Sync-Scripts laufen aus dem neuen Pfad. Alter Pfad als git-worktree-Quelle weiter nutzbar (deshalb existieren beide Pfade noch parallel). **Bekannter Stolperstein.** B-061 hat die Sync-Scripts portabel gemacht, P-umzug hat den Pfad geaendert — die Kombination muss bei jedem Doku-Sync robust sein. `sync_repos.ps1` testet seit B-061 sowohl alten als auch neuen Zerberus-Pfad. **Lessons.** (1) Worktree-Branches selbst auf main mergen (B-061 musste den Vorgaenger-Worktree-Branch selbst mergen, weil vorige Coda-Session ihn in mjolnir.md an Chris delegiert hatte). (2) OBERSTES GEBOT als Backstop (P-umzug Teil 1 — Anti-Pattern war genau dieser Worktree-Merge-Delegate-Verstoss).
