...
*Letzte Aktualisierung: P-debt-11 (2026-05-09) — Stand-Anker-Trim in `docs/huginn_kennt_zerberus.md` + Spiegel-Kopie. Schulden #6-Aufrueumung: alte Patch-Eintraege (P-UI-1..11 + P-UI-Polish-1/2 + P-debt-10) aus dem Stand-Anker auf eine kompakte Liste reduziert, FAQ-Block in die ersten 4000 Zeichen verschoben (Embedding-First-Chunk-Hypothese), Triple-Backtick-Code-Blocks entfernt (`compute_effort_score`-Code-Marker im P-UI-11-Eintrag + VRAM-Block in Inline-Text umgeschrieben), neue Sektion „Aktuelle Konfiguration (Live-Werte zur Laufzeit)" verweist auf den dynamisch generierten `[Aktuelle System-Informationen]`-Block aus P185. Heilt 4 huginn-doc-bezogene Failures (`test_p213_pre_4_huginn_ranking::test_faq_block_appears_near_top`, `test_patch169_bugsweep::test_doc_does_not_use_code_blocks`, `test_patch185_runtime_info::TestRagDocUpdate` 2x). Reine Doku-Wartung, kein Code-Pfad. Vorletzter Maintenance-Patch unveraendert P-debt-10. Letzter Backend-Bugfix-Patch unveraendert P217. Letzter Code-Patch mit UI-Mechanik unveraendert P-UI-11. Phase 5a + Phase 5c bleiben VOLLSTAENDIG ABGESCHLOSSEN.*
