...
*Letzte Aktualisierung: Patch 102 (2026-04-22)*

## Aktueller Patch
**Patch 102** – Kritische Bugfixes: Session-Isolation, Zustandsanzeige, Whisper-Filter, LLM-Fallback, Begrüßung, Input (2026-04-22)
- **Cluster 1 (B-02/B-06) Session-Isolation:** Bei Session-Wechsel kamen Antworten alter Sessions in der neuen an. Root-Cause war kein `AbortController` für laufende Chat-Requests — Backend filtert via SQL korrekt nach `session_id`, Frontend leert den Chat-Container, aber der laufende `fetch` zur alten Session lief weiter und seine Response landete in der jetzt aktiven Session. Fix in `nala.py`: globaler `currentChatAbort` (AbortController), Snapshot `reqSessionId` vor `fetch`, Stale-Check nach Response (`reqSessionId !== sessionId` → `[SESSION-102]`-Warning + verwerfen). `abortActiveChat()` in `loadSession`/`newSession`/`doLogout`/`handle401`. ✅
- **Cluster 2 (B-03/B-08/B-09/B-11/B-17) Zustandsanzeige:** Drei klare UI-Zustände implementiert. (1) **LÄUFT**: CSS-Spinner-Rad (`@keyframes spin`) statt der bisherigen Bouncing-Dots (Rad pausiert sichtbar bei UI-Freeze, Dots tun das nicht), Text "Antwort wird generiert…". `showTypingIndicator()` + `setTypingState('running')` werden SOFORT nach `fetch`-Start aufgerufen (vorher kam der Indikator nur via SSE-Event von `llm_start`). `lockInput()` deaktiviert `textarea` + Send-Button. (2) **TIMEOUT**: 45 s Frontend-Timeout via `AbortController`, Spinner pausiert (`.frozen`), Text "Keine Antwort vom Server", Inline-Retry-Button, `[TIMEOUT-102]`-Warning. (3) **FEHLER**: `error-bubble` (rote linke Border) bei NetworkError/5xx, Retry-Button, `[ERROR-102]`-Logging. `finally`-Block garantiert `unlockInput()` auch bei Exception. ✅
- **Cluster 3 (B-01) Whisper Phrasen-Repetition:** Bestehender `max_repetitions`-Block in `cleaner.py:47` filtert nur identische aufeinanderfolgende EINZELwörter ("ein ein ein") — Whisper-Loops wie "ein bisschen so ein bisschen so …" gingen ungehindert durch. Neue Funktion `detect_phrase_repetition()` arbeitet auf N-Grammen (Default 2-6 Wörter), greedy von der längsten Phrase abwärts. Konfigurierbar via `whisper_cleaner.repetition_filter` in `config.yaml` (`enabled`, `min_phrase_len`, `max_phrase_len`, `max_repeats`). `RepetitionFilterConfig` als Pydantic-Model in `config.py` ergänzt. `[WHISPER-REP-102]`-Warning bei Erkennung. **8 neue Unit-Tests** in `test_cleaner.py` — alle grün. ✅
- **Cluster 4 (B-20) LLM-Fallback absichern:** Diagnose ergab — keine `random.choice()`-Modellauswahl im Code (`llm.py:40` nutzt bereits `model_override or settings.legacy.models.cloud_model`), aber die `openrouter:`-Section fehlte komplett in `config.yaml`, sodass die Provider-Blacklist (Chutes, Targon) leer war. **Wichtige Architektur-Entscheidung:** `OpenRouterConfig.provider_blacklist` Default in `config.py:91` auf `["chutes", "targon"]` gesetzt — `config.yaml` ist gitignored, deshalb muss die Default-Blacklist im Code stehen, sonst greift sie nach `git clone` nicht. `[FALLBACK-102]`-WARNING-Logs an beiden Fallback-Pfaden in `legacy.py:258,265` (Orchestrator-Exception + Orchestrator-nicht-initialisiert). **3 neue Tests** via Source-Inspektion (robust gegen pytest-Loop-Konflikte mit Playwright). ✅
- **Cluster 5 (B-05) Begrüßung:** "Hallo, Nala!" Root-Cause war Patch-72-Logik in `nala.py:2733-2799`, die einen `char_name` aus dem System-Prompt extrahierte (`Du bist Nala`). Komplett refaktoriert: Backend `/nala/greeting` liefert jetzt nur noch `{prefix, name}` mit `name = display_name` aus dem User-Profil (also "Chris"), Frontend `fetchGreeting()` hat 4 GREETING_VARIANTS und pickt zufällig — echte Varianz pro Aufruf. ✅
- **Cluster 6 (B-07) Input-Verhalten Mobile-First:** Auf Mobile (Touch) macht Enter jetzt IMMER Zeilenumbruch (Soft-Keyboards können nicht zwischen Enter und Shift+Enter unterscheiden, deshalb wäre "Enter sendet" auf Mobile fatal — Senden geht ausschließlich über den Send-Button). Auf Desktop ist das Verhalten umschaltbar via Settings-Modal-Toggle (Default = "Enter sendet" für Patch-67-Kompatibilität). `localStorage 'zerberus_enter_sends'` persistiert die Wahl. `isTouchDevice()` Helper kombiniert `ontouchstart`, `navigator.maxTouchPoints` und `pointer: coarse` Media-Query. ✅
- **Tests:** 44/45 grün post-Restart. 11 neue Unit-Tests (8 cleaner + 3 llm_fallback) alle grün. Loki E2E (14 Tests) komplett grün. Fenrir Chaos: 19/20 — der eine ERROR (`test_chat_input_accepts_chaos[chromium-]` mit leerem String) ist ein **bestehender Setup-Bug** (Playwright/empty-string-Edge-Case), nicht durch Patch 102 verursacht.
- **Commit:** `74b72ee` — `Patch 102 – Session-Isolation, Zustandsanzeige, Whisper-RepFilter, LLM-Fallback, Begrüßung, Input-Fix`. Push erfolgreich nach `origin/main`.
- **Architektur-Lesson:** `config.yaml` ist gitignored — wenn ein neuer Config-Schlüssel fürs Repo essentiell ist (wie die Provider-Blacklist gegen unsichere Anbieter), gehört der Default in `config.py`, nicht nur in `config.yaml`. Sonst greift er bei jedem frischen Clone nicht.
